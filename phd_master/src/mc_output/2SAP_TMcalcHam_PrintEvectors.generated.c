...
struct hinge_span {	/* This data structure is used to store information about two-spans */
	...
	unsigned int		edgecount;	// total number of edges in poly1
	unsigned int		edgecount2;	// total number of edges in poly2
	unsigned int		hedges[vM*vL];	// contains info about what edges are occupied in the hinge (to avoid duplicates)
...
unsigned long int	*tspans_outsection[max_keynum+1];	/*An array of pointers to integers */
...
unsigned long int	*tspans_edges[max_keynum+1];	/*An array of pointers to integers*/
...
unsigned long int	*tspans_nrr[max_keynum+1];	/*An array of pointers to integers*/
...
unsigned int reordertemplate2[2][vM][vL];	//holds the re-ordered ordertemplate to reduce number of unique sections.
...
unsigned short int alreadyentered2[vM][vL];	//keeps track of where we've already initially entered (for 2nd poly). 0=no, 1=yes.
...
void            	enterhinge(int, int, int, int (*)[6]);
void            	leavehinge(int, int, int, int (*)[6]);
void            	rowedges(int, int, int (*)[6]);
void            	coledges(int, int, int (*)[6]);
void            	enterhinge2(int, int, int, int (*)[6]);
void            	leavehinge2(int, int, int, int (*)[6]);
void            	rowedges2(int, int, int (*)[6]);
void            	coledges2(int, int, int (*)[6]);
...
/*			printf("Section %d is valid, it connects to %lu other sections. \n", i, num_outsections[i]);
			printf("section %d consists of the two sections %lu and %lu\n", i, sectionkey2SAP[i][0], sectionkey2SAP[i][1]);
			printf("OG section nums of these are %lu and %lu\n", sectionkey[sectionkey2SAP[i][0]], sectionkey[sectionkey2SAP[i][1]]);
			printf("Section %d connects to these %lu other sections: ", i, num_outsections[i]);
			for(j=1; j<=num_outsections[i]; j++){
				printf("%lu ", tspans_outsection[i][j]);
			}
			printf("\n");
//			printsection(i);
*/			totalsecs++;
...
//	printf("OGsec1=%lu, OGsec2=%lu\n", sectionkey[sectionkey2SAP[testsection][0]], sectionkey[sectionkey2SAP[testsection][1]]);
//	printsection(sectionkey[sectionkey2SAP[testsection][0]]);
//	printsection(sectionkey[sectionkey2SAP[testsection][1]]);
...
//		printf("iter=%d\n", k);
	...
	if(M==2 && L==2){
		...
		for(k=1; k<=testpower; k++){
			...
			for(i=1; i<=max_keynum; i++){
				for(j=1; j<=num_outsections[i]; j++){
					temptestvector2[tspans_outsection[i][j]] += testvector2[i]*tspans_edges[i][j]; //temp=test*matrix
				}
			}
			for(i=1; i<=max_keynum; i++){
...
//		printf("Taking out section %lu, k=%d of %lu\n", nonconnectingsections[k], k, numnoconnect-1);
		for(j=1; j<=num_outsections[nonconnectingsections[k]]; j++){
			tspans_edges[nonconnectingsections[k]][j] = 0;						//CHANGED EDGES
			tspans_outsection[nonconnectingsections[k]][j] = 0;					//CHANGED OUTSECTION
		}
	...
	char filename3[100];		//this will hold the filename of the text file that we will read from
	sprintf(filename3, "TMinfoL%dM%d_Print.txt", L, M);
	FILE* fp3 = fopen(filename3, "w");
	for(i=1; i<=max_keynum; i++){
		for(j=1; j<=num_outsections[i]; j++){
			fprintf(fp3, "%d %d %lu, (%lu,%lu)->(%lu,%lu) OG:(%lu,%lu)->(%lu,%lu)\n", i, j, tspans_outsection[i][j], sectionkey2SAP[i][0], sectionkey2SAP[i][1], sectionkey2SAP[tspans_outsection[i][j]][0], sectionkey2SAP[tspans_outsection[i][j]][1], sectionkey[sectionkey2SAP[i][0]], sectionkey[sectionkey2SAP[i][1]], sectionkey[sectionkey2SAP[tspans_outsection[i][j]][0]], sectionkey[sectionkey2SAP[tspans_outsection[i][j]][1]]);
		}
	}
	...
	//set tspans_nrr properly now
	for(i=1; i<=max_keynum; i++){
		tspans_nrr[i] = unsgnlong_vecalloc(1, num_outsections[i]);
	}
	...
	for(ii=1; ii<=max_keynum; ii++){
		for (jj=1; jj<=num_outsections[ii];jj++){
			tspans_nrr[ii][jj] = ++tspan_num;
//			printf("ii=%lu, jj=%lu, entry=%lu\n", ii, jj, tspans_nrr[ii][jj]);
		}
	}
	...
	//calculates max eigenvalue
	double max_Evalue;
	max_Evalue = max_eval_LRvec(connectivity_inv) + 1; //max_eval_LRvec() returns max eigenvalue-1


	//EIGENVALUE/EIGENVECTOR CHECKS
/*
	printf("max eigenvalue=%f\n", max_Evalue);

	//check for zero entries
	for(i=1; i<=max_tspans; i++){
		if(R_Evector[0][i] < 0.00000000001){
			printf("entry %d in R_Evector is zero\n", i);
		}
	}
	...
	for(i=1; i<=1; i++){
		for(j=1; j<=num_outsections[i]; j++){	//tspan ij
			if(R_Evector[0][tspans_nrr[i][j]]>0.00000001){
				printf("\ntspan %lu connects sections %d->%lu\n", tspans_nrr[i][j], i, tspans_outsection[i][j]);
				printf("tspan %lu connects to %lu other tspans\n", tspans_nrr[i][j], num_outsections[tspans_outsection[i][j]]);
				sumofprobs=0.0;
				for(k=1; k<=num_outsections[tspans_outsection[i][j]]; k++){
					printf("connection to tspan %lu. Adding (1/%f)(R_Evector[%lu]/R_Evector[%lu]).\n", tspans_nrr[tspans_outsection[i][j]][k], max_Evalue, tspans_nrr[tspans_outsection[i][j]][k], tspans_nrr[i][j]);
					printf("= (1/%f)(%f/%f) = %f\n", max_Evalue, R_Evector[0][tspans_nrr[tspans_outsection[i][j]][k]], R_Evector[0][tspans_nrr[i][j]], 1/max_Evalue/R_Evector[0][tspans_nrr[i][j]]*R_Evector[0][tspans_nrr[tspans_outsection[i][j]][k]]);
					sumofprobs += 1/max_Evalue/R_Evector[0][tspans_nrr[i][j]]*R_Evector[0][tspans_nrr[tspans_outsection[i][j]][k]];
...
/*	for (i=1;i<= max_tspans;i++){
		L_Evector[0][i] = L_Evector[0][i] / LmultR;
		R_Evector[0][i] = R_Evector[0][i] / LmultR;
	}
*/

	//print L_Evector[0] and R_Evector[0] to file
	char filename[100];		//this will hold the filename of the text file that we want to write to
	char filename2[100];		//this will hold the filename of the text file that we want to write to
	sprintf(filename, "data/2SAP_L_EvectorHam_TS_L%dM%d.txt", L, M);
	sprintf(filename2, "data/2SAP_R_EvectorHam_TS_L%dM%d.txt", L, M);
	FILE* fp = fopen(filename, "w");
	FILE* fp2 = fopen(filename2, "w");
	for(i=1; i<= max_tspans; i++){
/*
		if(R_Evector[0][i] < 0.000000001){
			printf("tspan number %d has R_Evector zero.\n", i);
			for(j=1; j<=max_keynum; j++){
				for(k=1; k<=num_outsections[j]; k++){
					if(tspans_nrr[j][k]==i){
						printf("tspan %d consists of sections %d -> %lu\n", i, j, tspans_outsection[j][k]);
						printf("section %d consists of sections %lu,%lu and section %lu consists of sections %lu,%lu\n", j, sectionkey2SAP[j][0], sectionkey2SAP[j][1], tspans_outsection[j][k], sectionkey2SAP[tspans_outsection[j][k]][0], sectionkey2SAP[tspans_outsection[j][k]][1]);
						printf("OG: %lu, %lu -> %lu, %lu\n", sectionkey[sectionkey2SAP[j][0]], sectionkey[sectionkey2SAP[j][1]], sectionkey[sectionkey2SAP[tspans_outsection[j][k]][0]], sectionkey[sectionkey2SAP[tspans_outsection[j][k]][1]]);
						printsection(sectionkey[sectionkey2SAP[j][0]]);
						printsection(sectionkey[sectionkey2SAP[j][1]]);
						printsection(sectionkey[sectionkey2SAP[tspans_outsection[j][k]][0]]);
						printsection(sectionkey[sectionkey2SAP[tspans_outsection[j][k]][1]]);
					}
				}
			}
		}
*/
	...
	printf("When x is set to x=1, max evalue=%.15f\n", max_Evalue);
...
//	printf("R_Evector[0][1]=%f\n", R_Evector[0][1]);
	...
	printf("Prior to removal: Total sections=%lu, total 2spans=%lu\n", totalsecs, total2spans);
	printf("Final (after removal): Total sections=%lu, total 2spans=%lu\n", numconnect, actual2spantotal);
	printf("max_sections was set = %d\n", max_sections);
	printf("max_keynum was set = %d\n", max_keynum);
	printf("\n");
...
//	}
	...
	for(i=1; i<=max_keynum; i++){
		tspans_outsection[i] = unsgnlong_vecalloc(1, num_outsections[i]); //allocate memory
		tspans_edges[i] = unsgnlong_vecalloc(1, num_outsections[i]); //allocate memory
	}
...
//			printf("sec1key=%lu, sec2key=%lu\n", sec1key, sec2key);
			tspans_outsection[sec1key][curArrayEnt[sec1key]] = sec2key;
			tspans_edges[sec1key][curArrayEnt[sec1key]] = (*current_hinge_span[section_num]).edgecount + (*current_hinge_span[section_num]).edgecount2;
			curArrayEnt[sec1key]++;
		}
	}
...
//	for(i=1; i<=max_keynum; i++){
//		tspans_nrr[i] = unsgnlong_vecalloc(1, num_outsections[i]); //allocate memory
//	}
/*
	for (section_num=1; section_num<=max_keynum; section_num++){
...
void
enterhinge(int i, int j, int side, int (*pointordNum)[6])
{
...
void
leavehinge(int i, int j, int side, int (*pointordNum)[6])
				...
				for (ii = 0; ii <= M; ii++) { // these are the "vertical" locations of the vertices being considered
					for (jj = 0; jj <= L; jj++) { // these are the horizontal locations of the vertices being considered
						if (hingestatus[ii][jj] == 0) {
							...
							enterhinge2(ii, jj, side, pointordNum);
						}
...
void
rowedges(int i, int j, int (*pointordNum)[6])
{
...
void
coledges(int i, int j, int (*pointordNum)[6])
...
/***************************************************************************/
void
enterhinge2(int i, int j, int side, int (*pointordNum)[6])
{
	if( !(alreadyentered2[i][j]==1 && side==0) ){
	//	printf("entered hinge with poly2 (i=%d, j=%d, side=%d\n", i, j, side);
		hingestatus[i][j]=1;
		...
		leavehinge2(i, j, side ^ 1, pointordNum);
			/* the SAW leaves the hinge out the opposite side, hence (side ^ 1) is passed */
			/* if side = 1 then side ^ 1 = 0 */
			/* if side = 0 then side ^ 1 = 1 */
		rowedges2(i, j, pointordNum);
			/* horizontal edges are explored */
		coledges2(i, j, pointordNum);
			/* vertical edges are explored */
...
void
leavehinge2(int i, int j, int side, int (*pointordNum)[6])
/* called by: rowedges,coledges,enterhinge */
{
	if( !(alreadyentered2[i][j]==1 && side==0) ){
	//	printf("left hinge with poly2 (i=%d, j=%d, side=%d\n", i, j, side);
		int ii;	/*vertex number in vertical direction*/
		int jj;		/*vertex number in horizontal direction */
...
void
rowedges2(int i, int j, int (*pointordNum)[6])
{
	...
	if(j>0){
		if(hingestatus[i][j-1]==0){
			...
			(*pointordNum)[5]++;	/* count the edge in the hinge */
						...
						/* the hinge this would be an appropriate place to do so */
			rowhingeedges[i][j-1]=1;
			leavehinge2(i, j-1, 0, pointordNum);
				/* exit the hinge on side 0 with j moved*/
			leavehinge2(i, j-1, 1, pointordNum);
				/* exit the hinge on side 1 with j moved*/
			rowedges2(i, j-1, pointordNum);
				/* explore horizontal moves within the hinge */
			coledges2(i, j-1, pointordNum);
				/* explore vertical moves within the hinge */
			(*pointordNum)[5]--;
				/* decrement the number of edges in the hinge */
			hingestatus[i][j-1]=0;
...
void
coledges2(int i, int j, int (*pointordNum)[6])
/* called by rowedges,coledges */
{
//	printf("coledges called (i=%d, j=%d\n", i, j);
	if(i>0){
		if(hingestatus[i-1][j]==0){
			...
			(*pointordNum)[5]++;	/* count the edge in the hinge */
						...
						/* the hinge this would be an appropriate place to do so */
			colhingeedges[i-1][j]=1;
			leavehinge2(i-1, j, 0, pointordNum);
				/* exit the hinge on side 0 with j moved*/
			leavehinge2(i-1, j, 1, pointordNum);
				/* exit the hinge on side 1 with j moved*/
			rowedges2(i-1, j, pointordNum);
				/* explore horizontal moves within the hinge */
			coledges2(i-1, j, pointordNum);
				/* explore vertical moves within the hinge */
			(*pointordNum)[5]--;
				/* decrement the number of edges in the hinge */
			hingestatus[i-1][j]=0;
...
//	printf("NEWoutNum2=%d\n", outNum2);
	...
	unsigned short int	Num_edges = (*pointordNum)[0] + (*pointordNum)[2] - 1;
				...
				/*get */
	unsigned short int	Num_edges2 = (*pointordNum)[3] + (*pointordNum)[5] - 1;
...
//		printf("looking through linked list\n");
		...
		if(test->outorder==outNum){	//if same 2 sections (for first poly)
			if(test->inorder2==inNum2 && test->outorder2==outNum2){	//if same 2 sections (second poly)
				if(test->edgecount==Num_edges && test->edgecount2==Num_edges2){ //if number of edges for both also match
...
//		printf("actually recording\n");
		...
		(*current_hinge_span[inNum]).edgecount = Num_edges;	// record the number of edges in first section and in hinge (of 1st poly)
		(*current_hinge_span[inNum]).edgecount2 = Num_edges2;	// record the number of edges in first section and in hinge (of 2nd poly)
		for(i=0; i<=vM*vL-1; i++){
			(*current_hinge_span[inNum]).hedges[i]=temp_hedges[i];
		}

/*		printf("Recorded 2-span. Section %d to %d (1st poly) and %d to %d (2nd poly), with hedges: ", inNum, outNum, inNum2, outNum2);
		for(i=0; i<=vM*vL-1; i++){
...
#include "../../../phd_archive/src/analysis/statistics/get_Beta_ssCheck.c"
...
void printordtemp2(){
	int i, j;
	printf("printing ordertemplate2. Side 0:\n");
	for(i=0; i<=M; i++){
		for(j=0; j<=L; j++){
			printf("%d ", ordertemplate2[0][i][j]);
		}
		printf("\n");
	}

	printf("ordertemplate2 side1:\n");
	for(i=0; i<=M; i++){
		for(j=0; j<=L; j++){
			printf("%d ", ordertemplate2[1][i][j]);
		}
		printf("\n");
	}
}
...
void printreordtemp2(){
	int i, j;
	printf("printing reordertemplate2. Side 0:\n");
	for(i=0; i<=M; i++){
		for(j=0; j<=L; j++){
			printf("%d ", reordertemplate2[0][i][j]);
		}
		printf("\n");
	}

	printf("reordertemplate2 side1:\n");
	for(i=0; i<=M; i++){
		for(j=0; j<=L; j++){
			printf("%d ", reordertemplate2[1][i][j]);
		}
		printf("\n");
	}
}
...
#include "../../../phd_archive/src/sections/findsection.c"
#include "../../../phd_archive/src/sections/printsection.c"


#include "../../../phd_archive/src/analysis/statistics/sumtoone_ss.c"
#include "../../../phd_archive/src/analysis/statistics/prob_check_ss.c"
//#include "../sections/enterpatstspan_ss.c"
#include "../../../phd_archive/src/sections/enterpats_ss.c"
#include "../../../phd_archive/src/analysis/statistics/transprobcheck_ss.c"
#include "../../../phd_archive/src/analysis/statistics/probkspan_ss_sectionkey.c"
#include "../../../phd_archive/src/utils/avgtwostrings.c"
...
#include "../../../phd_archive/src/analysis/statistics/rtflsp.c"
...
#include "../../../phd_archive/src/utils/qcksrt_int.c"
...
#include "../../../phd_archive/src/utils/qcksrtII_int.c"
