...
//max_tspans is number of 2-poly (final) tspans.
#if M==1 && L==1
...
#define max_keynum 21546
#define max_tspans 1126578
...
void				printtspan(unsigned long int num);	//prints the 4 1-poly sections which are in tspan number "num"
...
*/
	...
	sprintf(filename, "data/2SAP_L_Evector_TS_L%dM%d.txt", L, M);
...
//	}
	...
	unsigned long int tspan_num=0,nth_outsection;
	for (section_num=1; section_num<=max_keynum; section_num++){
...
void printtspan(unsigned long int num){	//prints the sections which are in tspan number "num"
//	int i, j, k;
//	for(i=1; i<=max_keynum; i++){
//		for(
//	}
