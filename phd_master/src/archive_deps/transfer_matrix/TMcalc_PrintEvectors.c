...
*/
	...
	sprintf(filename, "data/L_Evector_TS_L%dM%d.txt", L, M);
	sprintf(filename2, "data/R_Evector_TS_L%dM%d.txt", L, M);
...
#include "pw_meth_ts_LRvec_fcheck.c" /* for applying the power method to the section-section matrix */
...
unsigned short int i,j;
	...
	for (i = 0; i <= M; i++) {
		for (j = 0; j <= L; j++)
			if(scanf("%i",&(ordertemplate[side][i][j]))){}
	}
}
