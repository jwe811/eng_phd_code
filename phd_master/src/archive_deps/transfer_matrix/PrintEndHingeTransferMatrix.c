/*
This program will output two text files--one containing the end-hinge section transfer matrix, with the other
containing the end-hinge 2span transfer matrix (for the specified tube size).
The text files will be called EndHingeSectionTransferMatrixL%dM%d.txt and EndHingeTspanTransferMatrixL%dM%d.txt
*/
...
#define	L 6				/* number of horizontal edges							*/
...
int num_right_endhinges[vec_length]; //given a section numbers (index), gives the number of endhinges which connect on the right
unsigned long int *endhinge_labels[vec_length];	//given a section number, nth entry is the nth right endhinge number.
...
void printsection(int secnum);	//prints a section
int numedgesinendhinge(int k);	//returns the number of edges in endhinge k
int edgesinsection(int sectionnum);	//returns number of edges in a section
...
/************************************************************************************************/
	...
	//PRINT END-HINGE SECTION TRANSFER MATRIX

	sprintf(filename, "EndHingeSectionTransferMatrixL%dM%d.txt", L, M);
...
/*
	for(i=1; i<=max_sections; i++){
		if(num_outsections[i]>0){
			printf("section %d connects to %d endhinges on the right. These endhinge numbers are: ", i, num_right_endhinges[i]);
			for(j=1; j<=num_right_endhinges[i]; j++){
				printf("%d, ", endhinge_labels[i][j]);
			}
			printf("\n");
		}
	}
*/
	int connects, addedup;
	for(i=1; i<=max_sections; i++){
		if(num_outsections[i]>0){ //row
			for(j=0; j<=total_end_hinges-1; j++){ //column
				connects=0;
				for(k=1; k<=num_right_endhinges[i]; k++){
					if(j==endhinge_labels[i][k]){
						connects=1;
					}
				}
				if(connects==0){
					...
					addedup = edgesinsection(i) + numedgesinendhinge(j);
					fprintf(fp, "x^%d ", addedup);
				}
			}
			fprintf(fp,"\n");
		}
	...
	fclose(fp);
	//PRINT END-HINGE 2SPAN TRANSFER MATRIX

	sprintf(filename, "EndHingeTspanTransferMatrixL%dM%d.txt", L, M);
	fp = fopen(filename, "w");	//create or overwrite the file "filename"
	...
	for(i=1; i<=max_sections; i++){
		if(num_outsections[i]>0){
			for(j=1; j<=num_outsections[i]; j++){	//row
				outsec = tspans_outsection[i][j];
				for(k=0; k<=total_end_hinges-1; k++){ //column
					connects=0;
					for(l=1; l<=num_right_endhinges[outsec]; l++){
						if(k==endhinge_labels[outsec][l]){
							connects=1;	//outsec connects to endhinge k on the right.
						}
					}
					if(connects==0){
						fprintf(fp, "0 ");
						...
						addedup = edgesinsection(outsec) + numedgesinendhinge(k);
						fprintf(fp, "x^%d ", addedup);
					}
				}				
				fprintf(fp,"\n");
...
conv_to_array(void) /* tspans_outsection[section_num][1..num_outsections[section_num]] */
{
	...
	for (section_num = 1; section_num <= max_sections; section_num++) {
		...
		while ((*current_hinge_span[section_num]).nexthinge != NULL) {
			...
			num_right_endhinges[current_hinge_span[section_num]->outorder] = current_hinge_span[section_num]->num_con_endhinges_right;
			endhinge_labels[current_hinge_span[section_num]->outorder] = unsgnlong_vecalloc(1, num_right_endhinges[section_num]);
			for(i=1; i<=num_right_endhinges[current_hinge_span[section_num]->outorder]; i++){
				endhinge_labels[current_hinge_span[section_num]->outorder][i] = current_hinge_span[section_num]->con_endhinges_right[i-1];
			}
