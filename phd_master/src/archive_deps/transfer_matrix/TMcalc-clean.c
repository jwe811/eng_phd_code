/*This program applies transfer matrix methods to polygons in an LxM tube
--Works for 1x1, 2x1, 3x1, 4x1, 5x1, 2x2, 3x2 tube sizes.
--Creates appropriate transfer matrix
--Find value of x (x_0) that results in a dominant eigenvalue of 1.
--Finds corresponding eigenvector.

Can compile with: gcc -lm -O3 -Wall -o TMcalc.o TMcalc.c
*/
...
#define	x_L 0.3	// left end point to use in bracketing method to find x_0
#define	x_R 0.8	// right end point to use in bracketing method to find x_0

#define	SS		// SS for section-section version, TS for tspan-tspan version

/** END OF DEFINITIONS THAT CHANGE DEPENDING ON SYSTEM BEING STUDIED **/


//Automatically set max_sections and max_tspans based on previous knowledge.
#if M==1 && L==1
...
double			*L_Evector[2]; 			/*An array of pointers to vectors*/
							/* L_Evector[0] denotes left eigenvector 0 */
							/* L_Evector[1] denotes left eigenvector 1 */ 
double			*R_Evector[2]; 			/*An array of pointers to vectors*/
...
double			**matalloc(int rowlow, int highrow, int lowcol, int highcol);
...
#include "../utils/vecalloc.c"		/*double * vecalloc(int low, int high)*/
/* used for dynamically allocating memory for vectors with enteries of type double */

#include "../utils/matalloc.c"		/*double **matalloc(int rowlow, int rowhigh, int collow, int colhigh)*/
/* used for dynamically allocating memory for matrices with enteries of type double */
