...
#else
#define vec_length (max_tspans+1)
#endif
...
*/
	...
	sprintf(filename, "L_EvectorHam_TS_L%dM%d_test.txt", L, M);
	sprintf(filename2, "R_EvectorHam_TS_L%dM%d_test.txt", L, M);
	...
	printf("\nWhen x is set to x=1, max evalue=%.15f\n", max_Evalue);
...
//	printf("R_Evector[0][1]=%f\n", R_Evector[0][1]);
	...
	printf("Final: Total sections=%lu, total 2spans (before reduction)=%lu\n", totalsecs, total2spans);
...
//#if L==2 && M==2
//	#include "../experiments/Trying.c" /* for applying the power method to the section-section matrix */
//#else
	#include "new_pw_meth_ts_LRvec_fcheck.c" /* for applying the power method to the section-section matrix */
//#endif
