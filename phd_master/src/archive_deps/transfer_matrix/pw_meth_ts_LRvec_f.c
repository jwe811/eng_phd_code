...
/**HERE**/
			...
			/*loop through sections which connect on the right*/
				...
				L_Evector[1][col_tspan_num] += pwfugacity*(L_Evector[0][row_tspan_num]); /*left  eigenvector*/
				R_Evector[1][row_tspan_num] += pwfugacity*(R_Evector[0][col_tspan_num]); /*right eigenvector*/
			}
		}
	}
...
/*for each two span that connects to this two span do*/
			...
			/*loop through sections which connect on the right*/
				...
				/*the following picks out the contribution to the resultant vectors in the */
				/*matrix multiplication*/
				L_Evector[0][col_tspan_num] += pwfugacity*(L_Evector[1][row_tspan_num]); /*left  eigenvector*/
				R_Evector[0][row_tspan_num] += pwfugacity*(R_Evector[1][col_tspan_num]); /*right eigenvector*/
			}
		}
	}
...
/*check for convergence*/
	...
	if ((-0.000001 < (L_lambda_1-L_lambda_0)) && ((L_lambda_1-L_lambda_0) < 0.000001)\
	   &&(-0.000001 < (R_lambda_1-R_lambda_0)) && ((R_lambda_1-R_lambda_0) < 0.000001)){
	printf("\n%6i (DONE)",2*k);
	printf("%13f %17f  ",L_lambda_1,L_lambda_1-L_lambda_0);
	printf("%16f %17f\n",R_lambda_1,R_lambda_1-R_lambda_0);
		return(R_lambda_1 - exp(-fval)); /*R_Evector[0] = right eigenvector corresponding to R_lambda_1*/
	}
