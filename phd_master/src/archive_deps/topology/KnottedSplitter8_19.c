...
main(void)
{
	...
	for(curfile=1; curfile<=totfiles; curfile++){
		...
		sprintf(PosTrefname, "ImpCreatorPrime_PKshrunkPolysL%dM%dspan%dnum%d_8_19.txt", L, M, span, curfile);	//CHANGEABLE
		...
		sprintf(NegTrefname, "ImpCreatorPrime_PKshrunkPolysL%dM%dspan%dnum%d_8_19s.txt", L, M, span, curfile);	//CHANGEABLE
