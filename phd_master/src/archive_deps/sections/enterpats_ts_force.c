//USES 2-span 2-span MATRIX TO FIND EXPECTED OCCURENCES FOR A 2-SPAN

double enterpats(double Beta, double LmultR, double kappa, double fval)
{				/* begin */
	printf("enterpats_ts\n");
	...
	unsigned short int lastedges;		//the most recent number of edges added
	...
	while (entering_patterns != 0) {
		...
		while (nth_section<=total_span) {	/* Accept user input
							 * until hinge edges
							 * entered = 0 */
			printf("nth_section=%d, total_span=%d, total_edges=%d\n", nth_section, total_span, total_edges);
					...
					while (hinge_validity == NOTVALID) {
						...
						for (nth_tspan = 1; nth_tspan <= num_outsections[section_num]; nth_tspan++) {
							if (tspan_num == tspans_nrr[section_num][nth_tspan] && \
								...
								lastedges = tspans_edges[section_num][nth_tspan];
							}
						}
	...
	printf("L_Evector[0][%d] = %e\n", first_tspan, L_Evector[0][first_tspan]);
	printf("R_Evector[0][%d] = %e\n", tspan_num, R_Evector[0][tspan_num]);
	printf("LmultR = %f\n", LmultR);


	Enum_Enum = (L_Evector[0][first_tspan]) * (R_Evector[0][tspan_num]) / LmultR;
	printf("L_Evector[%i]*R_Evector[%i] = %f = %e\n\n", first_tspan, tspan_num, Enum_Enum,Enum_Enum);
	printf("The expected number of times this %i-span pattern occurs\n", --nth_section);
	printf("in a polygon with span m is (as n->infinity):\n\n");
	printf("(%f)m = (%e)m\n", Enum_Enum * pow(connectivity_inv, total_edges-lastedges) * exp(fval*(total_span-2)) / Beta, Enum_Enum * pow(connectivity_inv, total_edges-lastedges) * exp(fval*(total_span-2)) / Beta);
