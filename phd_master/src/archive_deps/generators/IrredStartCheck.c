/*	This program will create ALL IRREDUCIBLE polygons in a LxM tube, with chosen span. Polygon must go the full span.	*/
...
#define	totalspan 6		/* span of the polygon that will be generated (must be >=3)	*/
...
/************************************************************************************************/
	...
	sprintf(filename, "NoOutput.txt", L, M, totalspan, filenum);
	...
	printf("firstsection=%d (out of %d total)\n", firstsection, max_sections);
		...
		while((*current_hinge_span[firstsection]).nexthinge != NULL){
			...
			if((*current_hinge_span[firstsection]).num_con_endhinges_left > 0){	//this tests to make sure we have a VALID starting 2-span (has a connecting endhinge on the left)
				...
				leftedges=(*current_hinge_span[firstsection]).numonleft;
...
/* which has been verified as being valid into a linked list */
{
	...
	(*current_hinge_span[inNum]).numonleft = (*pointordNum)[0] -1;
...
#include "recursivegenerate_IrredStartCheck.c"
#include "bfacf_irred_check.c"
