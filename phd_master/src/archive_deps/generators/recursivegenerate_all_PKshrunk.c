...
//THIS VERSION WILL USE bfacf_shrink() TO FIRST SHRINK POLYGON AND ONLY PRINT WITH >="minedges" EDGES
...
//				printf("this poly has %d edges\n", i);
				...
				if(i>=minedges){
					...
					if(filetotal>maxpolys){	//then we need to close up the current file, and create a new one to write to.
						...
						fclose(fp);
						filenum++;
						sprintf(filename, "CreatorAll_PKshrunkPolysL%dM%dspan%dnum%d.txt", passedL, passedM, totalspan, filenum);
