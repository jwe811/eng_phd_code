...
/** START OF DEFINITIONS THAT CHANGE DEPENDING ON SYSTEM BEING STUDYIED6 **/
#define	M 0				/* number of vertical edges                                */
#define	L 2				/* number of horizontal edges                              */
#define	vM 1				/* number of vertices in vertical direction   = M + 1      */
#define	vL 3				/* number of vertices in horizontal direction = L + 1      */
...
#define		CS     /* CS for column states when M=0 */

#define	maxOrdNum  7778			/* Maximum number assigned in num_section.c divided by eight*/
					/* and rounded up plus one to have array[0..number/8] */
					/* V6 => 139  ; V8 => 7778  ; V10 => 699942  */
#define	max_sections 62216		/* Maximum number assigned in num_section.c  */
...
#define	vec_length   62217		/* vector dimension for section matrix = max_sections + 1		*/
					/* This is because C assigns arrays of dimension n as [0..(n-1)]	*/

#define max_tspans   108		/* vector dimension for two-span matrix  		*/
...
/** END OF DEFINITIONS THAT CHANGE DEPENDING ON SYSTEM BEING STUDYIED **/
...
/**************************************************************************/
	...
	ordNum[0] = 1;	/* first edge in section 0 gets numbered 1   */
	ordNum[1] = 1;	/* first edge in section 1 gets numbered 1   */
	ordNum[2] = 0;	/* to start there are no edges in the 2-span */
...
#if defined(TS)
	L_Evector[0] = vecalloc(1,max_tspans); /*global*/
	L_Evector[1] = vecalloc(1,max_tspans); /*global*/ /*temp*/
	R_Evector[0] = vecalloc(1,max_tspans); /*global*/
	R_Evector[1] = vecalloc(1,max_tspans); /*global*/ /*temp*/
#else
	L_Evector[0] = vecalloc(1,max_sections); /*global*/
	L_Evector[1] = vecalloc(1,max_sections); /*global*/ /*temp*/
	R_Evector[0] = vecalloc(1,max_sections); /*global*/
	R_Evector[1] = vecalloc(1,max_sections); /*global*/ /*temp*/
#endif
...
enterhinge(bits_16 occstatebin, bits_16 hnodebin, int i, int j, int side, int (*pointordNum)[3])
{
	...
	ordertemplate[side][i][j] = 0;
		/* the egde that was recorded must now be erased as this edge may not exist */
...
{
	unsigned short int	inNum = num_section(0);
...
#include "../sections/Num_section_8V.c"  /*unsigned long int num_section(int side)*/
