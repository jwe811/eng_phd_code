//Run in SS mode. Creates TM with NO 2-string sections 
...
#define x_R 0.5			/* right end point to use in bracketing method to find x_0       */
...
/************************************************************************************************/
	...
	printf("check for irreducibility/periodicity\n");

	unsigned long int testsection=409;	//section that is going to be "tested" to see if it eventually can connect to all other sections
...
//Full version runs check here, for irreduciblity and "actual" valid 2span/sections counts
	...
	#if defined(force)
		printf("\n=> %f=%e is fugacity which yields 1 as maximum eigenvalue (after multiplying matrix by e^%f)\n",
					connectivity_inv=rtflsp(&max_eval_LRvec,x_L,x_R, 0.0000001), connectivity_inv, fval);
	#else
...
/* called by: rowedges,coledges,enterhinge */
{
	...
	if ((side == 0 && (*pointordNum)[0] > 3) &&  (*pointordNum)[1] > 3 ) {
 	/* if the leaving edge is on the left and there is exactly two edges on the left and right
