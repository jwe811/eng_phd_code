...
#define totalspan 2			//span of generated polygon (max span of 5 as of now. too many polygons with span=6 (>2,118,039,304). need to make changes to data types)
...
/**************************************************************************/
	...
	//perform check if endhinge connects to a sections (loop over all endhinges, and sections)
		//record this info...	num_connect[endhingenum] (stores the number of sections that connect to this endhinge)
		//			endhinge_sec[endhingenum][1...num_connect] (stores the section # of the ith section to connect to endhingenum)
...
//				printf("found %d polygons that start with section %d. Multiply this by %d for the different starthinges, and add this to total. we now have %d total polygons\n", sec_polygons, i, mult_starthinge, tot_polygons);
	...
	printf("\nTOTAL NUMBER OF POLYGONS THAT ARE IN A M=%d, L=%d tube, of span=%d are %d!!\n\n", M, L, totalspan, tot_polygons);
...
/* which has been verified as being valid into a linked list */
{
	...
	//calculate num_con_endhinges
	//find which endhinges connect to ordertemplate[0] (left side)
	//record their "number" in con_edges[num_con_endhinges]
...
//		printf("Will now add all endedges that contain exactly %d totedge(s)\n", totedges);
