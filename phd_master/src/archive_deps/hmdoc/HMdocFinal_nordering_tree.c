...
//#define	thex 0.361863	/* When the connectivity is known its inverse is defined here.   */
...
#define x_R 0.6			/* right end point to use in bracketing method to find x_0       */
...
//Automatically set max_sections
#if M==1 && L==1
#define max_sections 8
#define OGmaxsecs 1110
...
#define OGmaxsecs 62216

#elif M==1 && L==4
#define max_sections 9309
#define OGmaxsecs 5599530
...
#else
#define max_sections 186180	//just an estimate...
#define OGmaxsecs 739138092  
#endif
...
struct node {
//	unsigned long int keynum;	will create a sectionkey array with keys at the end.
	unsigned long int secnum;
	struct hinge_span* firsthinge;
	struct hinge_span* curhinge;
	struct node* left;
	struct node* right;
} *firstnode;
...
int reordertemplate[2][vM][vL];

int hingestatus[vM][vL];		//keeps track of which vertices in the hinge are occupied
int alreadyentered[vM][vL];	//keeps track of where we've already initially entered

int colhingeedges[M][vL];
int rowhingeedges[vM][vL];

unsigned long int specialint = OGmaxsecs/2;
...
struct node*		findnode(struct node*, unsigned long int, unsigned long int);
void				recordleaf(struct node* curnode, unsigned long int inNum, unsigned long int outNum, unsigned short int Num_edges, int temp_hedges[]);
int				recordnode(struct node* curnode, unsigned long int outNum, unsigned short int Num_edges, int temp_hedges[]);
...
/**************************************************************************/
	...
	printf("specialint is %lu\n", specialint);
	//make space for first node (root of tree) and its leaves.
	firstnode = newnode();

	firstnode->secnum = specialint;
...
//	printf("R_Evector[0][1]=%f\n", R_Evector[0][1]);
	...
	printf("Final: Total sections=%d, total 2spans=%d\n", totalsecs, total2spans);
...
*/
	...
	printf("want to record t-span with sections %d and %d\n", inNum, outNum);
	curnode = findnode(firstnode, inNum, 0);	//returns a pointer to the node that has secnum==inNum.
										//if inNum doesn't exist in the tree yet, then it returns the leaf that it will fill.
	if(curnode->secnum==0){	//leaf
		//record
		recordleaf(curnode, inNum, outNum, Num_edges, temp_hedges);		//fills curnode (leaf) with appropriate info
		sec_counter++;
		valid_2_spans++; /*This is a valid 2 span so it is counted*/
		printf("recorded a leaf\n");
	}
	else{	//already exists. check for dup and add to linked list
		int wasdup;
		wasdup = recordnode(curnode, outNum, Num_edges, temp_hedges);		//Checks for duplicate--adds to linked list in curnode
		if(wasdup==0){
			printf("recorded a non-duplicate\n");
			valid_2_spans++; /*This is a valid 2 span so it is counted*/
		}
		else{
			printf("recorded a duplicate\n");
		}
	}
	return;
}
...
struct node* newnode(void) {
	struct node* nextnewnode;
	nextnewnode = ((struct node *) calloc(1, sizeof(struct node)));
	if(nextnewnode==NULL){
		fprintf(stderr, "unable to allocate memory for a new node");
		exit(1);
	}
	nextnewnode->firsthinge = newhinge();
	nextnewnode->curhinge = nextnewnode->firsthinge;
	nextnewnode->secnum=0;
	return nextnewnode;
}
/***************************************************************************/
...
struct node* findnode(struct node* curnode, unsigned long int secnum, unsigned long int depth){
	unsigned long int testnum = curnode->secnum;
	struct node* nodetoreturn;

	if(testnum==0){	//if leaf
		printf("returning a node that's a leaf\n");
		return curnode;
	}
	else if(secnum==specialint && depth==0){	//special case: if at firstnode and secnum is the special int, go right.
		nodetoreturn = findnode(curnode->right, secnum, depth+1);
	}
	else if(secnum<testnum){
		printf("going left\n");
		nodetoreturn = findnode(curnode->left, secnum, depth+1);
	}
	else if(secnum>testnum){
		printf("going right\n");
		nodetoreturn = findnode(curnode->right, secnum, depth+1);
		...
		printf("returning a node that has a match\n");
		return curnode;
	}
	else{
		printf("this shouldn't run. EVER! exitting\n");
		exit(1);
	}
}

void recordleaf(struct node* curnode, unsigned long int inNum, unsigned long int outNum, unsigned short int Num_edges, int temp_hedges[]){	//fills curnode (leaf) with appropriate info
	curnode->secnum=inNum;
	curnode->curhinge->nexthinge = newhinge();
	curnode->curhinge = curnode->curhinge->nexthinge;
	curnode->curhinge->outorder = outNum;
	curnode->curhinge->edgecount = Num_edges;
	int i;
	for(i=0; i<=vM*vL-1; i++){
		curnode->curhinge->hedges[i]=temp_hedges[i];
	}
	curnode->left = newnode();
	curnode->right = newnode();
}

int recordnode(struct node* curnode, unsigned long int outNum, unsigned short int Num_edges, int temp_hedges[]){	//fills curnode (leaf) with appropriate info
	//check if this 2-span configuration has already been recorded
	int duplicate=0;
	int match;
	int i;
	struct hinge_span* test = curnode->firsthinge;
...
//		printf("actually recording\n");
		...
		printf("Recorded 2-span. Section %d to %d, with hedges: ", curnode->secnum, outNum);
		for(i=0; i<=vM*vL-1; i++){
			printf("%d ", curnode->curhinge->hedges[i]);
		}
		printf("\n");
	}
	return duplicate;
...
#include "../analysis/statistics/probkspan_ss3.c"
