...
unsigned short int testvector[MAX_KEYNUM_ARR+1];	//this will be the vector corresponding to the testsection.
unsigned short int temptestvector[MAX_KEYNUM_ARR+1];	//used for the vector-matrix multiplication
unsigned short int testvector2[MAX_KEYNUM_ARR+1];	//this will be the vector corresponding to the testsection.
unsigned short int temptestvector2[MAX_KEYNUM_ARR+1];	//used for the vector-matrix multiplication
...
//	unsigned long int testvector[MAX_KEYNUM_ARR+1];	//this will be the vector corresponding to the testsection. had to make this global for some reason.
...
//	unsigned long int temptestvector[MAX_KEYNUM_ARR+1];	//used for the vector-matrix multiplication. had to make this global for some reason
...
/////////
		for (i = 1; i <= num_left_endhinges[section_num]; i++){
			...
			for (j = 0; j < 3; j++){
				Lend_start[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Lend walk data");
		   		if (Lend_start[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}
				Lend_start2[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Lend walk data2");
		   		if (Lend_start2[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}

				Lend_end[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Lend walk data");
		   		if (Lend_end[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}
				Lend_end2[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Lend walk data2");
			...
			for (j = 0; j <= vM*vL; j++){
				Lend_walks[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Lend walk data");
			...
			for (j = 0; j <= vM*vL; j++){
				Lend_walks2[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Lend walk data2");
...
/////////
		for (i = 1; i <= num_right_endhinges[section_num]; i++){
			...
			for (j = 0; j < 3; j++){
				Rend_start[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Rend walk data");
		   		if (Rend_start[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}
				Rend_start2[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Rend walk data2");
		   		if (Rend_start2[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}

				Rend_end[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Rend walk data");
		   		if (Rend_end[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}
				Rend_end2[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Rend walk data2");
			...
			for (j = 0; j <= vM*vL; j++){
				Rend_walks[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Rend walk data");
		   		if (Rend_walks[section_num][i][j] == NULL) {
					fprintf(stderr, "Out of memory");
					exit(0);
				}
				Rend_walks2[section_num][i][j] = (int*)generated_xcalloc(MAX_vMvL + 1, sizeof(int), "Rend walk data2");
...
/* this is a memory allocation function */
{
	...
	for(i=0; i<=2; i++){
		...
		nextnewendhinge->start2[i] = int_vecalloc(0,max_walks-1);
		nextnewendhinge->end2[i] = int_vecalloc(0,max_walks-1);
	}
	for(i=0; i<=vM*vL; i++){
		nextnewendhinge->walks[i] = int_vecalloc(0,numberofwalks-1);
		nextnewendhinge->walks2[i] = int_vecalloc(0,max_walks-1);
	}
