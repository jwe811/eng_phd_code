...
# Source files
TM_SRC = $(SRCDIR)/MASTER_TMcalc.c $(SRCDIR)/tm_spectral.c $(SRCDIR)/run_metadata.c
...
$(BUILDDIR)/%.tm.o: $(SRCDIR)/%.c $(INCDIR)/tm_runtime.h $(INCDIR)/tm_spectral.h $(INCDIR)/run_metadata.h
...
test: directories $(TM_OUT)
	...
	PYTHONDONTWRITEBYTECODE=1 python3 scripts/audit_engine.py -L 1 -M 1 -m 0
