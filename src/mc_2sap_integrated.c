...
//			printf("looking at section composed of sections %d and %lu (on the left)\n", i, mc2_current_hinge_span[i]->inorder2);
			pair_key = mc_2sap_pair_map_get(&pair_map, (unsigned long int)i, mc2_current_hinge_span[i]->inorder2);
			if(pair_key!=0){
				mc2_num_outsections[pair_key]++;
			}
			else{	//need to record sections into mc2_sectionkey2SAP
				keycounter++;
				if((unsigned long int)keycounter > max_keynum){
					fprintf(stderr, "Fatal: exceeded 2SAP key capacity (%lu)\n", max_keynum);
					exit(EXIT_FAILURE);
				}
				pair_key = (unsigned long int)keycounter;
				mc2_sectionkey2SAP[pair_key][0] = (unsigned long int)i;
				mc2_sectionkey2SAP[pair_key][1] = mc2_current_hinge_span[i]->inorder2;
				mc2_num_outsections[pair_key]++;
				mc_2sap_pair_map_put(&pair_map, (unsigned long int)i, mc2_current_hinge_span[i]->inorder2, pair_key);
			}	
		}
	}
...
//			printf("this hinge is made up of sections %lu,%lu -> %lu,%lu\n", section_num, (*mc2_current_hinge_span[section_num]).inorder2, (*mc2_current_hinge_span[section_num]).outorder, (*mc2_current_hinge_span[section_num]).outorder2);
			...
			//get keynums
			sec1key=mc_2sap_pair_map_get(&pair_map, section_num, (*mc2_current_hinge_span[section_num]).inorder2);
			if(sec1key==0){
				printf("error: didn't find sec1key. Exitting\n");
				exit(1);
			}
			sec2key=mc_2sap_pair_map_get(&pair_map, (*mc2_current_hinge_span[section_num]).outorder, (*mc2_current_hinge_span[section_num]).outorder2);
			if(sec2key==0){
...
void mc2_conv_endhinges_to_array(void)
{
	...
	mc_2sap_pair_map_build(&pair_map, max_keynum, mc2_sectionkey2SAP);
	...
	for (section_num = 1; section_num <= max_sections; section_num++) {
		...
		while ((*mc2_currentendhinge[section_num]).nextendhinge != NULL) {
			...
			//get keynum
			sec1key=mc_2sap_pair_map_get(&pair_map, section_num, (*mc2_currentendhinge[section_num]).sec2);
			if(sec1key==0){
				printf("error: didn't find sec1key. Exitting\n");
				exit(1);
			}
...
/* this is a memory allocation function */
{
	...
	nextnewhinge = (struct hinge_span *)mc_xcalloc(1, sizeof(struct hinge_span), "2SAP hinge span");
	mc_2sap_init_int_rows(nextnewhinge->start, 3, numberofwalks, "2SAP hinge starts");
	mc_2sap_init_int_rows(nextnewhinge->end, 3, numberofwalks, "2SAP hinge ends");
	mc_2sap_init_int_rows(nextnewhinge->start2, 3, numberofwalks2, "2SAP hinge starts2");
	mc_2sap_init_int_rows(nextnewhinge->end2, 3, numberofwalks2, "2SAP hinge ends2");
	mc_2sap_init_int_rows(nextnewhinge->walks, (size_t)vM * (size_t)vL + 1, numberofwalks, "2SAP hinge walks");
	mc_2sap_init_int_rows(nextnewhinge->walks2, (size_t)vM * (size_t)vL + 1, numberofwalks2, "2SAP hinge walks2");
...
/* this is a memory allocation function */
{
	...
	nextnewendhinge = (struct endhinge *)mc_xcalloc(1, sizeof(struct endhinge), "2SAP endhinge");
	mc_2sap_init_int_rows(nextnewendhinge->start, 3, numberofwalks, "2SAP endhinge starts");
	mc_2sap_init_int_rows(nextnewendhinge->end, 3, numberofwalks, "2SAP endhinge ends");
	mc_2sap_init_int_rows(nextnewendhinge->start2, 3, max_walks, "2SAP endhinge starts2");
	mc_2sap_init_int_rows(nextnewendhinge->end2, 3, max_walks, "2SAP endhinge ends2");
	mc_2sap_init_int_rows(nextnewendhinge->walks, (size_t)vM * (size_t)vL + 1, numberofwalks, "2SAP endhinge walks");
	mc_2sap_init_int_rows(nextnewendhinge->walks2, (size_t)vM * (size_t)vL + 1, max_walks, "2SAP endhinge walks2");
...
static Mc2SapSampleWriterConfig mc2_sample_writer_config(void)
{
	...
	const Mc2SapModeSpec *mode_spec = mc_2sap_mode_spec(0);
...
static double mc2_max_eval_LRvec(double fugacity)
{
	return mc_2sap_max_eval(max_keynum, (unsigned long int)max_tspans, mc2_num_outsections,
		...
		mc2_MC_L_Evector, mc2_MC_R_Evector, fval, L, M, mc_2sap_mode_spec(0)->hamiltonian, fugacity);
}
