...
#if defined(CS)
#include "../deps/archive/topology/LFlag_0.c"		/* include if M = 0  */
...
#include "../deps/archive/utils/noncrossing.c"	/* include if M = 0 */
...
#else
#include "../deps/archive/topology/LFlag_norder.c"		/* include if M <> 0 */
/* This function takes an arguement (*pointordNum)[3] */
/* it returns 1 if the two-span being considered connects to phi on the left and returns 0 otherwise */
/* it also uses the global variable ordertemplate */
#include "../deps/archive/topology/LFlag_endhinge_norder.c"

#define tspans_outsection t_outsection
#define tspans_nrr t_nrr
#define R_Evector R_Evector_solve
#include "../deps/archive/transfer_matrix/pw_meth_ts_LRvec_fcheck.c"
...
#include "../deps/archive/utils/unsgnlong_vecalloc.c" /*unsigned long int unsgnlong_vecalloc(int low, int high)*/
/* used for dynamically allocating memory for vectors with unsigned long integer enteries */

#include "../deps/archive/utils/vecalloc.c"		/*vec_ent * vecalloc(int low, int high)*/
/* used for dynamically allocating memory for vectors with enteries of type vec_ent */

#include "../deps/archive/utils/matalloc.c"		/*mat_ent **matalloc(int rowlow, int rowhigh, int collow, int colhigh)*/
