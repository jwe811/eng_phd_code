...
#if defined(CS)
#include "mc_helpers/topology/LFlag_0.c"		/* include if M = 0  */
...
#include "mc_helpers/utils/noncrossing.c"	/* include if M = 0 */
...
#else
#include "mc_helpers/topology/LFlag_norder.c"		/* include if M <> 0 */
/* This function takes an arguement (*pointordNum)[3] */
/* it returns 1 if the two-span being considered connects to phi on the left and returns 0 otherwise */
/* it also uses the global variable ordertemplate */
#include "mc_helpers/topology/LFlag_endhinge_norder.c"

#define tspans_outsection t_outsection
#define tspans_nrr t_nrr
#define R_Evector R_Evector_solve
#include "mc_helpers/transfer_matrix/pw_meth_ts_LRvec_fcheck.c"
