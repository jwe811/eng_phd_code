...
main(void)
{
	...
	const int span = 6;	//CHANGEABLE
	const int num_files = 12;	//CHANGEABLE (number of files)
	...
	for(curfile=1; curfile<=num_files; curfile++){
		...
		sprintf(filenametwo, "ImpCreatorTwoStringsPrime_PKshrunkL%dM%dspan%dnum%d.txt_shrunkID", L, M, span, curfile);	//CHANGEABLE
