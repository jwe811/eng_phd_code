//Takes a polygon file and the corresponding knot ID file
//Creates a new txt file that will contain only the trefoils (pos or neg).
//NOT FINISHED
...
main(void)
{
	...
	char InterestName[100];	//files with the polygons
	char KnotIDName[100];	//files with the knot ID's corresponding to arcs from polygons (patterns) of interest
	char NewName[100];		//files where local knots will go

	char line[7];		//line in Interest
	char IDone[10];	//The knotID
