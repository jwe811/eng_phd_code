/*	This program will create ALL polygons in a LxM tube, with chosen span, that are "patterns of interest", with the pattern being a SLIPKNOT. This means their
start and end sections only have two edges. Polygon must go the full span. Slipknot will mean that the polygon is unknotted, but when looking at one of the arcs, it
forms a knot */
...
#define	totalspan 5		/* span of the polygon that will be generated (must be >=3)	*/
...
/************************************************************************************************/
	...
	sprintf(filename, "SlipknotPolysL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
	...
	fptwo = fopen("tracking_slipknot.txt", "w");
	if(fptwo != NULL){
		fprintf(fptwo, "Starting to create polys\n");
		printf("tracking_slipknot.txt is created\n", filename);
	}
	else{
	...
	for(firstsection=1; firstsection<=max_sections; firstsection++){
		if(test2secs(firstsection)==1){	//ONLY WANT POLYGONS WITH 2 SECTIONS IN THE FIRST SECTION
			fptwo = fopen("tracking_slipknot.txt", "a");
	...
	*/
	...
	if(filenum==1){
		...
		printf("Number of Polys OF INTEREST with >=%d edges AFTER BEING SHRUNK = %d = %f = %e!\n", minedges, filetotal, (float)filetotal, (float)filetotal);
	...
	printf("\nDone generating all polygons of interest with >= %d edges AFTER BEING SHRUNK in %f seconds.\n\n", minedges, (double)clock()/(double)CLOCKS_PER_SEC);
	fclose(fp);
...
#if defined(CS)
#include "LFlag_0.c"		/* include if M = 0  */
...
#include "RFlag_0.c"		/* include if M = 0  */
/* This function takes an arguement (*pointordNum)[3] */
...
#else
#include "LFlag.c"		/* include if M <> 0 */
/* This function takes an arguement (*pointordNum)[3] */
/* it returns 1 if the two-span being considered connects to phi on the left and returns 0 otherwise */
/* it also uses the global variable ordertemplate */

#include "RFlag.c"		/* include if M <> 0 */
/* This function takes an arguement (*pointordNum)[3] */
/* it returns 1 if the two-span being considered connects to phi on the right and returns 0 otherwise */
/* it also uses the global variable ordertemplate */
...
#include "../generators/recursivegenerate_slipknot.c"
