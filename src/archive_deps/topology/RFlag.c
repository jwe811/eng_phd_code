...
{
	if ((*pointordNum)[1] - 1 < vM*vL) {	/* if there are less edges in first section */
...
/* But since (*pointordNum) is odd and vM*vL is even it is not necessary */
		...
		for (i = 0; i <= M; i++) {
			...
			for (j = 0; j < L; j++) {/*the i and j loops serve to loop through*/
				...
				switch ((ordertemplate[1][i][j]) - (b=ordertemplate[1][i][j + 1])) {
				case (1):{
		...
		for (i = 0; i < M; i++) {
			...
			for (j = 0; j <= L; j++) {/*the i and j loops serve to loop through*/
				...
				switch ((ordertemplate[1][i][j]) - (b=ordertemplate[1][i+1][j])) {
				case (1):{
						if ( (b & 1) == 1) /*b is smaller and congruent to 1 mod 2*/
						return(1);
						break;
