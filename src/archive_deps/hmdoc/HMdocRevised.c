...
#define x_R 1.1			/* right end point to use in bracketing method to find x_0       */
...
*/
	...
	#if defined(force)
		printf("\n=> %f is fugacity which yields e^-%f=%f as maximum eigenvalue\n",
					connectivity_inv=rtflsp(&max_eval_LRvec,x_L,x_R, 0.0000001), fval, exp(-fval));
	#else
...
#if defined(TS)
	#if defined(force)
		#include "../transfer_matrix/pw_meth_ts_LRvec_f.c" /* for applying the power method to the 2-span 2-span matrix */
	#else
...
#if defined(SS)
	#if defined(force)
		#include "../transfer_matrix/pw_meth_ss_LRvec_f.c" /* for applying the power method to the section-section matrix */
	#else
