...
#define		vec_ent float	/*vector entries will be of type defined here*/
#define		mat_ent float	/*matrix entries will be of type defined here*/
...
//////////CHECK IF THE TRANSMATRIX IS IRREDUCIBLE////////////////////
	...
	unsigned long int valsec[valid_sections];	//array that will hold the valid section numbers
	...
	unsigned long int testpower=101;	//power of the matrix that will be multiplied by testvector (steps in markov chain)
	for(k=1; k<=testpower; k++){
		...
		for(i=1; i<=max_sections; i++){
			for(j=1; j<=num_outsections[i]; j++){
				temptestvector[tspans_outsection[i][j]] += testvector[i]*pow(connectivity_inv, tspans_edges[i][j]); //temp=test*matrix
			}
		}
		for(i=1; i<=max_sections; i++){
	...
	unsigned long int numnoconnect=0;	//this will count the number of non-connected sections
	printf("Section %d doesn't connect to sections: ", testsection);
	for(i=1; i<=max_sections; i++){
		for(j=0; j<=valid_sections-1; j++){
			if(i==valsec[j]){		//if section i is in valsec, then check if testvector[i] is zero
				if(testvector[i] ==0){	// check if testvector[i] is 0
					printf("%d ", i);
					numnoconnect++;
				}
			}
	...
	k=0;
	unsigned long int notconnected[numnoconnect];	//contains the sections that aren't connected to testsection
	for(i=1; i<=max_sections; i++){
		for(j=0; j<=valid_sections-1; j++){
			if(i==valsec[j]){		//if section i is in valsec, then check if testvector[i] is zero
				if(testvector[i] ==0){	// check if testvector[i] is 0
					notconnected[k]=i;	//record section number of section that isn't connected to testsection
	...
	for(i=1; i<=max_sections; i++){		//loop through all sections
		for(k=0; k<=numnoconnect-1; k++){	//loop through all sections that don't connect
			if(i==notconnected[k]){		//if section i is not connected to testsection
				num_outsections[i]=0;							//CHANGED NUM_OUT
				for(j=1; j<=num_outsections[i]; j++){
					tspans_edges[i][j] = 0;						//CHANGED EDGES
					tspans_outsection[i][j] = 0;					//CHANGED OUTSECTION
				}
				//FINISHED CHANGING FOR WHEN INSECTION=i
				//NOW MAKE CHANGES FOR WHEN OUTSECTION=i
				for(j=1; j<=max_sections; j++) {	//loop through all sections
					for(m=1; m<=num_outsections[j]; m++){	//loop through all of the outsections
						if(tspans_outsection[j][m]==i){		//if section j has section i as an outsection, need to make changes
							dummy=m;
							while(dummy<num_outsections[j]){	//while there are additional outsections
								tspans_outsection[j][dummy] = tspans_outsection[j][dummy+1];	//CHANGED OUTSECTION
								tspans_edges[j][dummy] = tspans_edges[j][dummy+1];		//CHANGED EDGES
								dummy++;
							}
							tspans_outsection[j][dummy] = 0;		//
							tspans_edges[j][dummy] = 0;
							num_outsections[j]--;				//CHANGED NUM_OUT
							m--;
						}
					}
				}
	...
	//now perform power method with changed num_out, t_edges, t_outsect

//	double newfug = rtflsp(&max_eval_LRvec,x_L,x_R, 0.0000001);	//new fugacity after changes.
//	printf("\nThe previous x_0 was %f, new x_0 after taking out sections that aren't connected to testsection=%d is %f\n\n", connectivity_inv, testsection, newfug);


//TEST IF CHANGES WERE CORRECT
	for(i=1; i<=max_sections; i++){
		for(j=0; j<=valid_sections-1; j++){
			if(i==valsec[j]){
				for(k=1; k<=num_outsections[i]; k++){
//					printf("t_edges[%d][%d] = %d, ", i, k, tspans_edges[i][k]);
//					printf("t_outsection[%d][%d] = %d\n", i, k, tspans_outsection[i][k]);
					for(m=0; m<=numnoconnect-1; m++){
						if(tspans_outsection[i][k]==notconnected[m]){
							printf("FOUND THAT SECTION %d HAS OUTSECTION=%d=%d\n", i, tspans_outsection[i][k], notconnected[m]);
							printf("k(nth-outsection)=%d, num_outsections[%d]=%d\n", k, i, num_outsections[i]);
						}
					}
				}
				break;
...
/*
	...
	double transmatrix[max_sections+1][max_sections+1];	//transition matrix
	for(i=1; i<=max_sections; i++){
		for(j=1; j<=max_sections; j++){
			transmatrix[i][j]=0;	//initialize the transmatrix to be all zeros
		}
	}

//	printf("TRANS MATRIX (section-section):\n");
	for(i=1;i<=max_sections;i++) {
		for(j=1;j<=num_outsections[i];j++) {
			transmatrix[i][tspans_outsection[i][j]] = pow(connectivity_inv, tspans_edges[i][j]);	//fill transmatrix with the proper entry
//			printf(" ENTRY (%d,%d)=%f. tspan #%d.\n", i, tspans_outsection[i][j], pow(connectivity_inv, tspans_edges[i][j]), tspans_nrr[i][j]);
		}
	}
...
//		printf("testvector[%d] = %f\n", i, testvector[i]);
	...
	for(k=1; k<=testpower; k++){
		...
		for(i=1; i<=max_sections; i++){
			for(j=1; j<=max_sections; j++){
				temptestvector[i] += testvector[j]*transmatrix[j][i]; //temp=matrix*test
			}
		}
		for(i=1; i<=max_sections; i++){
			testvector[i]=temptestvector[i];	//set test=temp
	...
	unsigned long int connectcount=0;
	printf("Section %d connects to sections: ", testsection);
	for(i=1; i<=max_sections; i++){
		for(j=0; j<=valid_sections-1; j++){
			if(i==valsec[j]){		//if section i is in valsec, then check if testvector[i] is zero
				if(testvector[i] !=0){	// check if testvector[i] is 0
	...
	printf("\nSection %d connects to %d other sections\n", testsection, connectcount);
	printf("\n");

	k=0;	//this will count the number of non-connected sections
	unsigned long int notconnected[valid_sections-connectcount];	//contains the sections that aren't connected to testsection
	for(i=1; i<=max_sections; i++){
		for(j=0; j<=valid_sections-1; j++){
			if(i==valsec[j]){		//if section i is in valsec, then check if testvector[i] is zero
				if(testvector[i] ==0){	// check if testvector[i] is 0
	...
	printf("Section %d isn't connected to %d sections:", testsection, k);
	for(i=0; i<=k-1; i++){
		printf(" %d", notconnected[i]);
	}
	printf("\n");


	//row and column of sections that aren't connected are changed to zero in transmatrix

/*	for(i=0; i<=k-1; i++){
		for(j=1; j<=max_sections; j++){
			transmatrix[notconnected[i]][j]=0;
			transmatrix[j][notconnected[i]]=0;
		}
	}
	printf("\nTRANSMATRIX HAS CHANGED TO CONNECTED MATRIX\n\n");
*/
...
/*
	...
	//perform power method on new transmatrix
	double pwtolerance = 0.0000001;
	double domevector[max_sections+1];
	double temp[max_sections+1];
	double norm = 1;
	double prevnorm = 1;
	double normSq = 0;
	...
	do {
		// find temp = transmatrix*evector
		for(i=1; i<=max_sections; i++){
			temp[i]=0;
			for(j=1; j<=max_sections; j++) {
				temp[i] += transmatrix[i][j] * domevector[j];	//dot product of the ith row with evector
			}
		}

		// calculate the length of temp
		prevnorm = norm;
		normSq=0;
		for(i=1; i<=max_sections; i++){
		...
		norm = sqrt(normSq);
		for(i=1; i<=max_sections; i++) {
			domevector[i] = temp[i]/norm;
		}
		iter++;
		printf("ITERATION: %d, norm = %f, prevnorm = %f, fabs(norm-prevnorm) = %f\n", iter, norm, prevnorm, fabs(norm-prevnorm));
		fflush(stdout);

	} while(fabs(norm-prevnorm) >= pwtolerance);


*/
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

//////////FINISHED CHECK IF THE TRANSMATRIX IS IRREDUCIBLE////////////////////
