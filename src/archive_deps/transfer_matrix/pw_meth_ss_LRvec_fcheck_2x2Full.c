...
printf("2x2 COMPACT SPECIAL CASE. GOING TO USE MAKE CHANGES TO USE A MATRIX WHICH IS IRREDUCIBLE AND APERIODIC\n");
...
L_Evector[0][1]=1;
R_Evector[0][1]=1;
for (i=2;i<=max_sections;i++) { L_Evector[0][i]=0; R_Evector[0][i]=0;}
*/
...
/**/	/* L_Evector[1] = L_Evector[0] * Matrix, then L_Evector[0] = L_Evector[1] * (I + Matrix) / 2		*/
	/* R_Evector[1] = Matrix * R_Evector[0], then R_Evector[0] = (I + Matrix) / 2 * R_Evector[1]		*/
...
//			printf("insection=%d, outsection=%d, edges=%d\n", insection_num, nth_outsection, tspans_edges[insection_num][nth_outsection]);
	...
	//NOW DO:
	// L_Evector[1] * (I-Matrix)/2
	// (I-Matrix)/2 * R_Evector[1]
	for (i = 1; i <= max_sections; i++) { /*initialize resultant vectors to zero */
		L_Evector[0][i] = 0;
		R_Evector[0][i] = 0;
	}
...
//			printf("insection=%d, outsection=%d, edges=%d\n", insection_num, nth_outsection, tspans_edges[insection_num][nth_outsection]);
			//MAKE CHANGE HERE, USE (I-MATRIX)/2 INSTEAD OF MATRIX
			if(insection_num==tspans_outsection[insection_num][nth_outsection]){	//must make I-Matrix correction
				pwfugacity = (1-pow(fugacity, tspans_edges[insection_num][nth_outsection]))/2;
				/*raises fugacity to number of edges */
			}
			else{
				pwfugacity = (-pow(fugacity, tspans_edges[insection_num][nth_outsection]))/2;
			}
	...
	//Evector[0] is now the resultant.

	for (i=1;i<=max_sections;i++){/*check for largest component in resultant vectors*/
		if (L_Evector[0][i] > L_lambda_0) { L_lambda_0 =  L_Evector[0][i]; }
		if (R_Evector[0][i] > R_lambda_0) { R_lambda_0 =  R_Evector[0][i]; }
		/*and set eigenvalue (ie. eventually converges to eigenvalue) to largest component*/
	}

	for (i=1;i<=max_sections;i++) { L_Evector[0][i] /= L_lambda_0; R_Evector[0][i] /= R_lambda_0; }
...
/**/	/* L_Evector[1] = L_Evector[0] * Matrix, then L_Evector[0] = L_Evector[1] * (I + Matrix) / 2		*/
	...
	for (i=1;i<=max_sections;i++){ L_Evector[1][i] = 0; R_Evector[1][i] = 0; }
	...
	/* loop through all possible sections */
		...
		/* for each section loops through the sections it connects to */
			...
			L_Evector[1][outsection_num] += pwfugacity*(L_Evector[0][insection_num])*exp(fval);
			R_Evector[1][insection_num] += pwfugacity*(R_Evector[0][outsection_num])*exp(fval);
		}
	}
