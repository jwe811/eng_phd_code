...
//SS version doesn't work for 2x2. Need to implement TS version for 2x2 to work. (works for printing Evectors).
...
#define x_L 0.58	/* left end point to use in bracketing method to find x_0        */
#define x_R 0.61	/* right end point to use in bracketing method to find x_0       */
...
*/			totalsecs++;
	...
	printf("Check (before removal): Total sections=%lu, total 2spans=%lu\n", totalsecs, total2spans);
...
exit(0);
	...
	printf("Prior to removal: number of saved 2-spans=%lu\n", total2spans);
exit(0);
	printf("Number of duplicate 2-spans generated that were not recorded=%lu\n", dupcounter);
	printf("Final number of 2-spans (after removal)=%lu\n", actual2spantotal);
...
#include "pw_meth_ss_LRvec_fcheck_HAM.c" /* for applying the power method to the section-section matrix */
