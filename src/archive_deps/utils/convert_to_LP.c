/*
This program takes in a name of a UofS polygon file (parameter) and will convert it to a polygon
...
int main(int argc, char* argv[]){
	...
	const int span=250;
	const int run=1;
	const int numfiles = 100;	//total number of files
	const int maxlen=10000;	//max length of a polygon

	int i,j;
	char filename[100];

	for(j=1; j<=numfiles; j++){
		sprintf(filename, "MC2SAPsHamL%dM%dspan%drun%dnum%d.txt", L, M, span, run, j);
