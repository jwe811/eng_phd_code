/*	This program will count ALL polygons (with no twostrings) in a LxM tube, with certain edge length.  */
...
#define	totaledges 20		/* Number of edges in polygons to be counted	*/
...
struct hinge_span {	/* This data structure is used to store information about two-spans */
	...
	int validstart;	//1 if it can start a polygon (w/o causing duplicate), 0 otherwise

	int edge_one;	//edges in first sec
	int edge_two;	//edges in hinge
	int edge_three; //edges in last sec
...
int cur_one; //as 2-spans are being generated, holds the cur number of edges
int cur_two;
int cur_three;

unsigned long int* edges_in_endhinge;
int curlength=0;
...
/**************************************************************************/
	...
	//CREATE ARRAY THAT HOLDS THE NUMBER OF EDGES IN EACH ENDHINGE.
	edges_in_endhinge = unsgnlong_vecalloc(0, total_end_hinges-1);

	for(i=0; i<=total_end_hinges-1; i++){
		edges_in_endhinge[i]=0;
		currentendhinge[i] = firstendhinge[i];
		while(currentendhinge[i] != NULL){
			edges_in_endhinge[i]++;
	...
	for(i=0; i<=total_end_hinges-1; i++){
		printf("endhinge %d has %d edges\n", i, edges_in_endhinge[i]);
	}
...
/************************************************************************************************/
	...
	//TIME TO COUNT SOME POLYGONS! YAYYYYY  ^^;;
	printf("TIME TO COUNT SOME POLYGONS!!!!!\n");
	...
	printf("firstsec=%d, ", firstsection);
		...
		while((*current_hinge_span[firstsection]).nexthinge != NULL){
			...
			if((*current_hinge_span[firstsection]).num_con_endhinges_left > 0){	//this tests to make sure we have a VALID starting 2-span (has a connecting endhinge on the left)
				if((*current_hinge_span[firstsection]).validstart==1){	//not a duplicate
...
//					printf("Adding first 2-span edges (section=%d). Previously=%d, now: ", firstsection, curlength);
					//Add edges for 2-span
					curlength = curlength + (*current_hinge_span[firstsection]).edge_one;
					curlength = curlength + (*current_hinge_span[firstsection]).edge_two;
//					printf("%d\n", curlength);

					//next, add count of edges for all possible endhinges
					for(curendhinge=1; curendhinge <= (*current_hinge_span[firstsection]).num_con_endhinges_left; curendhinge++){	//for each endhinge that connects
						...
						//Add edges for endhinge.
//						printf("Adding left endhinge edges (endhinge %d). Previously=%d, now: ", (*current_hinge_span[firstsection]).con_endhinges_left[curendhinge-1], curlength);
						curlength = curlength + edges_in_endhinge[(*current_hinge_span[firstsection]).con_endhinges_left[curendhinge-1]];
//						printf("%d\n", curlength);


						//CLOSE OFF IMMEDIATELY (SPAN=2 POLY)
						if(current_hinge_span[firstsection]->num_con_endhinges_right>0){	//then add appropriate edges and see if it has the correct amount.
					//		printf("there are %d right endhinges that work with this polygon.\n", current_hinge_span[firstsection]->num_con_endhinges_right);
							int curendhinge_other;
							for(curendhinge_other=1; curendhinge_other <= current_hinge_span[firstsection]->num_con_endhinges_right; curendhinge_other++){	//for each endhinge that connects
								curlength = curlength + current_hinge_span[firstsection]->edge_three; //last section's edges.
								curlength = curlength + edges_in_endhinge[current_hinge_span[firstsection]->con_endhinges_right[curendhinge_other-1]];
	...
	//						printf("thirdsection=%d\n", nexttspan->outorder);
							recursivegenerate(2, nexttspan, nextsection, x, y);	//all polygons now generated with this starting 2-span.
						}
						//reset for different endhinge
						curlength = curlength - edges_in_endhinge[(*current_hinge_span[firstsection]).con_endhinges_left[curendhinge-1]];
					}
					//reset for different first section
					curlength = curlength - (*current_hinge_span[firstsection]).edge_one;
					curlength = curlength - (*current_hinge_span[firstsection]).edge_two;
				}
/*				else{
					printf("section %d is a duplicate\n", firstsection);
...
//	printf("\ncurlength=%d\n", curlength);	//should be zero

	printf("\nL=%d, M=%d, Total edges=%d\n", L, M, totaledges);
	printf("Number of polys (with no 2-strings) with %d edges: %d million + %d\n", totaledges, millions, grand);

	printf("\nDone counting all polygons in %f seconds.\n\n",(double)clock()/(double)CLOCKS_PER_SEC);
...
/* called by: rowedges,coledges,enterhinge */
{
	...
	if ((side == 0 && (*pointordNum)[0] > 3) && (*pointordNum)[1] > 3) {
...
/* which has been verified as being valid into a linked list */
{
	...
	(*current_hinge_span[inNum]).edgecount = Num_edges + (*pointordNum)[0] -1;	/* record the number of edges in first section and in hinge */
	(*current_hinge_span[inNum]).edge_one = (*pointordNum)[0] -1;
	(*current_hinge_span[inNum]).edge_two = (*pointordNum)[2];	//edges in hinge.
	(*current_hinge_span[inNum]).edge_three = (*pointordNum)[1] -1;

	//check if this can be a "starting 2-span" that doesnt result in a duplicate (ie, first edge is labelled lexicographically)
	int validstart=0;
	for(j=0; j<=M; j++){
		for(i=0; i<=L; i++){  // Lexi ordering: (0,0) (1,0) ... (L,0) (0,1) (1,1) ... (L,1) ...
			if(ordertemplate[0][j][i] == 1){
				validstart=1;
				break;
			}
			else if (ordertemplate[0][j][i] != 0){
				validstart=-1;
				break;

			}
		}	
		if(validstart==1 || validstart==-1){
			break;
		}
	}
	if(validstart==-1){ validstart=0; }
	(*current_hinge_span[inNum]).validstart = validstart;
