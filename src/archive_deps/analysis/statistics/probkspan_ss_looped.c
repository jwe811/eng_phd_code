...
double probkspan(double Beta, double LmultR, double kappa, FILE* fpWrite)
{
...
//	Edges[2] = #edges in k-span (not including last section)
	...
	while(filenum<=totalfilenum){	//while there are files to read in
		...
		sprintf(filename, "LocalTwoStrings_PKshrunkL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
