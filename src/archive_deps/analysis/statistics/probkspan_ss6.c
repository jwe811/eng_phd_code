...
void probkspan6(double Beta, double LmultR, double kappa)
{
	printf("probkspan6() (section version) is running!\n");
...
//	Edges[2] = #edges in k-span (not including last section)
	...
	while(filenum<=totalfilenum){	//while there are files to read in
		...
		sprintf(filename, "TwoStringsNotFullL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
