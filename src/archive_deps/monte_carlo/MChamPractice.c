/*	This program will create ALL polygons in a LxM tube, with chosen span. Polygon must go the full span.	*/
...
struct endhinge {			/* an endhinge is a linked list of edges			*/
	...
	int gennum;			//order in which the endhinge was generated.
};
...
char 			filename2[36];			//this will hold the filename of the text file that we want to write to
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	sprintf(filename, "MChamPracticePolysL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
...
//	printf("right end hinges=%d\n", num_right_endhinges);
//	printf("left end hinges=%d\n", num_left_endhinges);
	...
	int* outsecs;
	double* EvecValues;
	double currentEvecValue;
	FILE* fp2;	//will be used to read in eigen vector values
	sprintf(filename2, "RvectorL%dM%dHam.txt", L, M);
	fp2 = fopen(filename2, "r");
	if(fp2 == NULL){
		printf("file pointer fp2 is pointing to NULL\n");
		exit(1);
	}

	int readint=0;
	double readdouble=1.2;
	...
	for(p=1; p<=NUM_SAMPLES; p++){

		choice = (rand() % num_right_endhinges) + 1;
		printf("choice is: %d\n", choice);

		foundsec=0;
		for(firstsection=1; firstsection<=max_sections; firstsection++){
			currentendhinge[firstsection] = firstendhinge[firstsection];
			while((*currentendhinge[firstsection]).nextendhinge != NULL){
				currentendhinge[firstsection] = (*currentendhinge[firstsection]).nextendhinge;
				if(currentendhinge[firstsection]->gennum==choice && currentendhinge[firstsection]->side==0){
					printf("found endhinge with gennum %d on left side. The section number is %d\n", choice, firstsection);
					foundsec=1;
	//				printsection(firstsection);
					break;
				}
			}
			if(foundsec==1){
		...
		printf("Section %d connects to endhinge %d on the left\n", firstsection, choice);
		...
		printf("there are %d ways out of section %d\n", waysout, firstsection);
		if(waysout==0){
			printf("this section cannot be part of a poly. exitting for now\n");
			exit(1);
		}

		choice = (rand() % waysout) + 1;
		printf("choice is: %d\n", choice);

		//point current at the choice'th hinge span
		current_hinge_span[firstsection] = first_hinge_span[firstsection];	//get current pointing to first
		hs_counter=0;
		while((*current_hinge_span[firstsection]).nexthinge != NULL && hs_counter<choice){
			current_hinge_span[firstsection] = (*current_hinge_span[firstsection]).nexthinge;	//current is now pointing at a possible starting 2-span
			hs_counter++;
		}

		nextsection = (*current_hinge_span[firstsection]).outorder;
		printf("The nextsection is %d\n", nextsection);
	...
	*/
	...
	//////////////////////////////////////////////////////////////////////////////
		...
		//At this point, Section "nextsection" has been generated. Poly has span=2. Need to start sampling according to eigenvector.

		
		for(curspan = 2; curspan<totalspan; curspan++){
			//need to add another section to "nextsection"
			currentsection = nextsection;

			//sample according to MC probability.
			waysout=0;
			current_hinge_span[currentsection] = first_hinge_span[currentsection];	//get current pointing to first
			while((*current_hinge_span[currentsection]).nexthinge != NULL){
				current_hinge_span[currentsection] = (*current_hinge_span[currentsection]).nexthinge;	//current is now pointing at a possible starting 2-span
				waysout++;
		
			}
			printf("there are %d ways out of section %d\n", waysout, currentsection);
			if(waysout==0){
				printf("this section cannot be part of a poly. exitting for now\n");
				exit(1);
			}

			outsecs = (int*) malloc(waysout * sizeof(int));	//array that holds the section numbers of the waysout.

			i=0;
			current_hinge_span[currentsection] = first_hinge_span[currentsection];	//get current pointing to first
			while((*current_hinge_span[currentsection]).nexthinge != NULL){
				current_hinge_span[currentsection] = (*current_hinge_span[currentsection]).nexthinge;	//current is now pointing at a possible starting 2-span
				outsecs[i] = (*current_hinge_span[currentsection]).outorder;
				i++;
			}

			printf("outsec array is filled with: ");
			for(i=0; i<waysout; i++){
				printf("%d ", outsecs[i]);
			}
			printf("\n");
			...
			EvecValues = (double*) malloc(waysout * sizeof(double));	//array that holds the eigenvector values of the sections in outsecs.


			for(i=0; i<waysout; i++){
				rewind(fp2);
				fscanf(fp2, "%d %lf\n", &readint, &readdouble);
//				printf("readint=%d, readdouble=%.11f\n", readint, readdouble);
				while(readint != outsecs[i]){
					fscanf(fp2, "%d %lf\n", &readint, &readdouble);
				}

				EvecValues[i]=readdouble;
				printf("Section %d=%d has a vector value of %.11f=%.11f\n", outsecs[i], readint, EvecValues[i], readdouble );
			}

			rewind(fp2);
			fscanf(fp2, "%d %lf\n", &readint, &readdouble);
//			printf("readint=%d, readdouble=%.11f\n", readint, readdouble);
			while(readint != currentsection){
				fscanf(fp2, "%d %lf\n", &readint, &readdouble);
			}

			currentEvecValue=readdouble;
			printf("currentsection %d=%d has a vector value of %.11f=%.11f\n", currentsection, readint, currentEvecValue, readdouble );
			...
			current_hinge_span[currentsection] = first_hinge_span[currentsection];	//get current pointing to first
			hs_counter=0;
			while((*current_hinge_span[currentsection]).nexthinge != NULL && hs_counter<choice){
				current_hinge_span[currentsection] = (*current_hinge_span[currentsection]).nexthinge;	//current is now pointing at a possible starting 2-span
				hs_counter++;
			}

			nextsection = (*current_hinge_span[currentsection]).outorder;
			printf("connecting section %d to section %d\n", currentsection, nextsection);




			addtowalk(printablewalk, (*current_hinge_span[currentsection]).walk);
			
		printf("Span is %d, printable walk is now: ", curspan+1);
		i=0;
		while(printablewalk[i]!=0){
			printf("%d ", printablewalk[i]);
			i++;
...
/* called by: rowedges,coledges,enterhinge */
{
	...
	if(RFlag_endhinge(&EndOrdNum)==1){
		for(loopi=0; loopi<=M; loopi++){
			for(loopj=0; loopj<=L; loopj++){
				if(endstatus[loopi][loopj] == 0){	// if vertex i,j is empty
					fullhinge=0;
					break;
				}
			}
...
*/
	...
	(*currentendhinge[secnum]).gennum = num_right_endhinges;
	//record walk
	i=0;
	while(curwalk[i]!=0){
...
*/
	...
	(*currentendhinge[secnum2]).gennum = num_right_endhinges;
	//record walk
	i=0;
	while(curwalk[i]!=0){
		if(curwalk[i]==1){
...
#include "../generators/Improvedaddtowalk.c"
#include "../generators/ImprovedRecurGenPrime.c"
