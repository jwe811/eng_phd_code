...
LFlag_endhinge2()
{
	...
	if (EndOrdNum[0]+EndOrdNum2[0] - 2 < vM*vL){
		...
		if(L==1 && M==1){ //then if they don't criss cross, return 1.
			if(endtemplate[0][0] != 0 && endtemplate[1][1] == 0){	//then they dont crisscross and we're ok.
				return(1);
			}
			else if(endtemplate3[0][0] != 0 && endtemplate3[1][1] == 0){	//then 2nd poly doesnt crisscross and we're ok
				return(1);
			}
		}
		else{
				...
				for (i = 0; i <= M; i++) {
					for (j = 0; j < L; j++) { /*the i and j loops serve to loop through*/
						...
						if(endtemplate[i][j]!=0 && endtemplate[i][j + 1]!=0){
							if(endtemplate[i][j] - endtemplate[i][j + 1]==EndOrdNum[0]-1-1){
								return(1);
							}
							else if(endtemplate[i][j] - endtemplate[i][j + 1]==1-(EndOrdNum[0]-1)){
								return(1);
							}
						}
					}
				...
				for (i = 0; i < M; i++) {
					for (j = 0; j <= L; j++) {/*the i and j loops serve to loop through*/
						...
						if(endtemplate[i][j]!=0 && endtemplate[i+1][j]!=0){
							if(endtemplate[i][j] - endtemplate[i+1][j]==EndOrdNum[0]-1-1){
								return(1);
							}
							else if(endtemplate[i][j] - endtemplate[i+1][j]==1-(EndOrdNum[0]-1)){
								return(1);
							}
						}
					}
			...
			if(EndOrdNum2[0]-1 >2){
				...
				for (i = 0; i <= M; i++) {
					for (j = 0; j < L; j++) { /*the i and j loops serve to loop through*/
						...
						switch ((endtemplate3[i][j]) - (b=endtemplate3[i][j + 1])) {
							/* look at the difference between the numbers of the two edges */
						...
						if(endtemplate3[i][j]!=0 && endtemplate3[i][j + 1]!=0){
							if(endtemplate3[i][j] - endtemplate3[i][j + 1]==EndOrdNum2[0]-1-1){
								return(1);
							}
							else if(endtemplate3[i][j] - endtemplate3[i][j + 1]==1-(EndOrdNum2[0]-1)){
								return(1);
							}
						}
					}
				...
				for (i = 0; i < M; i++) {
					for (j = 0; j <= L; j++) {/*the i and j loops serve to loop through*/
						...
						switch ((endtemplate3[i][j]) - (b=endtemplate3[i+1][j])) {
						...
						if(endtemplate3[i][j]!=0 && endtemplate3[i+1][j]!=0){
							if(endtemplate3[i][j] - endtemplate3[i+1][j]==EndOrdNum2[0]-1-1){
								return(1);
							}
							else if(endtemplate3[i][j] - endtemplate3[i+1][j]==1-(EndOrdNum2[0]-1)){
								return(1);
							}
						}
					}
