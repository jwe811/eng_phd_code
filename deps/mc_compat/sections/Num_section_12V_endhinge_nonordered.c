/***************************************************************************/
unsigned long int
num_section_endhinge_norder(void)
{
	...
	unsigned short int slot_num[13];	/*of the positions available to the edge determined by*/
...
for (i=1;i <= 12;i++){slot_num[i]=0;} /* This is necessary to initialize the array to contain zero elements */

	for (i = 0; i <= M; i++) {
		for (j = 0; j <= L; j++) {
			edge_num=rendtemplate[i][j];/*assign number of edge determined by i and j to edge_num */
...
if (slot_num[11] !=0){
	return(61594841.0*slot_num[1]-67824659.0+5599531.0*slot_num[2]+559953.0*slot_num[3]+62217.0*slot_num[4]+7777.0*slot_num[5]+1111.0*slot_num[6]+185.0*slot_num[7]+37.0*slot_num[8]+9.0*slot_num[9]+3.0*slot_num[10]+slot_num[11]);
}
if (slot_num[9] !=0){
	return(61594841.0*slot_num[1]-67824659.0+5599531.0*slot_num[2]+559953.0*slot_num[3]+62217.0*slot_num[4]+7777.0*slot_num[5]+1111.0*slot_num[6]+185.0*slot_num[7]+37.0*slot_num[8]+9.0*slot_num[9]+3.0*slot_num[10]);
}
if (slot_num[7] !=0){
	return(61594841.0*slot_num[1]-67824648.0+5599531.0*slot_num[2]+559953.0*slot_num[3]+62217.0*slot_num[4]+7777.0*slot_num[5]+1111.0*slot_num[6]+185.0*slot_num[7]+37.0*slot_num[8]);
}
if (slot_num[5] !=0){
	return(61594841.0*slot_num[1]-67824427.0+5599531.0*slot_num[2]+559953.0*slot_num[3]+62217.0*slot_num[4]+7777.0*slot_num[5]+1111.0*slot_num[6]);
}
if (slot_num[3] !=0){
	return(61594841.0*slot_num[1]-67816540.0+5599531.0*slot_num[2]+559953.0*slot_num[3]+62217.0*slot_num[4]);
}
	return(61594841.0*slot_num[1]-67194371.0+5599531.0*slot_num[2]);
...
/***************************************************************************/
unsigned long int
num_section_endhinge_norder2(void)
{
	unsigned long int i,j; 			/*used as dummy variables in for loops */
	unsigned short int edge_num;		/*the number of the edge being considered in for loop*/
						/*this is zero if there is no edge there */
...
for (i=1;i <= 12;i++){slot_num[i]=0;} /* This is necessary to initialize the array to contain zero elements */
	...
	for (i = 0; i <= M; i++) {
		for (j = 0; j <= L; j++) {
			edge_num=rendtemplate2[i][j];/*assign number of edge determined by i and j to edge_num */
							   /*this will be zero if there is no edge */
