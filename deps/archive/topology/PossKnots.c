/*	This program will create ALL polygons in a LxM tube with >="minedges" edges (36 FOR 2x1), with chosen span. Polygon must go the full span.	*/
...
/************************************************************************************************/
	...
	sprintf(filename, "PossKnotsPolysL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
...
*/
	...
	*/
	...
	printf("\nDone generating all possibly knotted polygons in %f seconds.\n\n",(double)clock()/(double)CLOCKS_PER_SEC);
	fclose(fp);
...
#include "../generators/recursivegenerate_PossKnots.c"
