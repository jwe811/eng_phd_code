...
//uses TMcalcHam_PrintEvectors.c, which prints out eigenvectors to:
//L_EvectorHam_TS_L%dM%d.txt
//R_EvectorHam_TS_L%dM%d.txt
//note: only uses R_Evector
//These files must be in same folder.

//compliled with: gcc -lm -Wall -O3 -o MCsample_Ham.o MCsample_Ham.c
//ran using ./MCsample_Ham.o

//sample SAPs will go to files: MCpolysHamL%dM%dspan%dnum%d.txt
...
//HAMILTONIAN NOT WORKING FOR 2x2 YET

#define	L 2				// number of horizontal edges
#define	M 1				// number of vertical edges
#define	totalspan 300		// span of sampled SAPs
#define	samplesize 1000000		// number of samples desired
#define	runnum 1
#define 	seednum 21300

#define	maxpolys 10000	// number of SAPs per file.
...
#define max_tspans 22
#define dom_evalue 3.732050810014727

#elif M==1 && L==2
#define max_sections 73
#define max_tspans 649
#define dom_evalue 14.076438172824952

#elif M==1 && L==3
#define max_sections 742
#define max_tspans 19554
#define dom_evalue 49.643407510907970

#elif M==1 && L==4
#define max_sections 9309
#define max_tspans 728925
#define dom_evalue 172.714480164060546

#elif M==1 && L==5
#define max_sections 138038
#define max_tspans 32294131
#define dom_evalue 596.477267188525502

#elif M==2 && L==2
#define max_sections 2619
#define max_tspans 184574
#define dom_evalue 104.488979151088131

#elif M==2 && L==3
#define max_sections 138322
#define max_tspans 39671908
#define dom_evalue 880.559222610926781
#endif
...
unsigned long int	*t_outsection[vec_length];	/* An array of pointers to integers 					*/
										/* records the "outsection" of the two-span				*/
										/* t_outsection[insection][nth_tspan]				*/
										/* nth_tspan = nth two-span with firstsection = insection	*/

unsigned long int	*t_nrr[vec_length];		/*An array of pointers to integers*/
...
unsigned int		*t_num_walks[vec_length];	//t_num_walks[insection][nth_tspan] holds the number of walks in this tspan

int				***t_start[vec_length];	//t_start[insection][nth_tspan][i][nth_walk] holds the i-coordinate (0=x, 1=y, 2=z) of the start point of the nth_walk

int				***t_end[vec_length];	//t_end[insection][nth_tspan][i][nth_walk] holds the i-coordinate (0=x, 1=y, 2=z) of the end point of the nth_walk

int				***t_walks[vec_length];	//t_end[insection][nth_tspan][direc][nth_walk] holds the "direc'th" direction the nth_walk



////////////////////// arrays which hold the endhinge info ////////////////////////////////////

unsigned long int	num_left_endhinges[vec_length];

unsigned int		*Lend_num_walks[vec_length];

int				***Lend_start[vec_length];

int				***Lend_end[vec_length];

int				***Lend_walks[vec_length];


unsigned long int	num_right_endhinges[vec_length];

unsigned int		*Rend_num_walks[vec_length];

int				***Rend_start[vec_length];

int				***Rend_end[vec_length];

int				***Rend_walks[vec_length];
...
unsigned short int testvector[vec_length];	//this will be the vector corresponding to the testsection.
unsigned short int temptestvector[vec_length];	//used for the vector-matrix multiplication
unsigned short int testvector2[vec_length];	//this will be the vector corresponding to the testsection.
unsigned short int temptestvector2[vec_length];	//used for the vector-matrix multiplication
...
//REMOVE SECTIONS/2-SPANS THAT CANNOT OCCUR IN A HAMILTONIAN SAP
...
//	unsigned long int temptestvector[vec_length];	//used for the vector-matrix multiplication. had to make this global for some reason
	...
	unsigned long int testpower=50;	//power of the matrix that will be multiplied by testvector (steps in markov chain)
...
//		printf("iter=%d\n", k);
	...
	unsigned long int nonconnectingsections[max_sections];		//this will hold the section numbers which DONT connect to testsection
	unsigned long int connectingsections[max_sections];		//this will hold the section numbers which DO connect to testsection
...
//	printf("Section %d doesn't connect to sections: \n", testsection);
	...
	for(i=1; i<=max_sections; i++){	//loop through all valid sections
...
//	printf("oddeven=%d\n", oddeven);
	printf("\n\n");
	printf("ACTUAL 'VALID' SECTIONS FOR THE L=%d, M=%d COMPRESSED CASE IS: %lu (down from %d).\n", L, M, numconnect, max_sections);
	...
	printf("ACTUAL 'VALID' 2SPANS FOR THE L=%d, M=%d COMPRESSED CASE IS: %lu (down from %lu).\n", L, M, actual2spantotal, num_tspans);
	printf("\n\n");
...
*/
	...
	printf("\nNOW SAMPLING: %d samples from L=%d, M=%d, span=%d (Hamiltonian)\n", samplesize, L, M, totalspan);

	sprintf(filename, "MCpolysHamL%dM%dspan%drun%dnum%lu.txt", L, M, totalspan, runnum, filenum);
...
//	sprintf(filename, "L_EvectorHam_TS_L%dM%d.txt", L, M);
	sprintf(filename2, "R_EvectorHam_TS_L%dM%d.txt", L, M);
...
//	printf("numerator=%d. denominator=%lu\n", 1, tot_left_endhinges);
//	printf("chance = %f\n", chance);
//	printf("1/chance = %f\n\n", 1/chance);
...
//				printf("curLEH=%d. secnum=%lu, nth_endhinge=%d\n", curLEH, secnum, nth_endhinge);
				if(curLEH==chosenLEH){
//					printf("\nchosenLEH=%d was found. Section=%lu, nth_endhinge=%d\n", chosenLEH, secnum, nth_endhinge);
...
//	printf("1/chance = %f\n\n", 1/chance);
						...
						curspan=1;
//						printf("Left endhinge has been added to built_walks:\n");
//						printbuiltwalks();
//						printf("\n");
...
//	printf("chance = %f\n", chance);
//	printf("1/chance = %f\n\n", 1/chance);
//						printf("first tspan chosen: section=%lu, nth_tspan=%d (possible [%d,%lu]), outsection=%lu, tspan_num=%lu\n", sec1, nth_tspan, 1, num_outsections[sec1], t_outsection[sec1][nth_tspan], t_nrr[sec1][nth_tspan]);
						add_to_built_walks(sec1, nth_tspan);	//add nth_tspan from sec1 to built_walks
//						printf("First tspan has been added, here is the result:\n");
//						printbuiltwalks();
//						printf("\n");
...
//								printf("sec %lu -> sec %lu. tspan_num=%lu. probability=%f. cumulative prob=%f\n", sec2, t_outsection[sec2][i], t_nrr[sec2][i], R_Evector[t_nrr[sec2][i]] / R_Evector[cur_tspan_num] / dom_evalue, sumofprobs);
							...
							if(sumofprobs > 1.1 || sumofprobs<0.9){
								...
								printf("tspan %lu connects to %lu other tspans\n", cur_tspan_num, num_outsections[sec2]);
								sumofprobs=0.0;
								for(i=1; i<=num_outsections[sec2]; i++){
//									printf("sumofprobs=%f\n", sumofprobs);
//									printf("Adding %f\n", R_Evector[t_nrr[sec2][i]] / R_Evector[cur_tspan_num] / dom_evalue);
									sumofprobs = sumofprobs + R_Evector[t_nrr[sec2][i]] / R_Evector[cur_tspan_num] / dom_evalue;
...
//							printf("tspan has been added (span=%d), here is the result:\n", curspan);
//							printbuiltwalks();
	//						printf("\n");
...
//						printf("LEH %d rejected\n", curLEH);
	...
	printf("\nRESULTS: %lu samples created from L=%d, M=%d, span=%d (Hamiltonian)\n", curSample, L, M, totalspan);
...
//		printf("num_outsections[%lu] = %lu\n", section_num, num_outsections[section_num]);
...
//			printf("section_num=%lu, ", section_num);
//			printf("t_num_walks[%lu][%d]=%d\n", section_num, i, t_num_walks[section_num][i]);
...
//		printf("section_num=%lu\n", section_num);
//	t_start[1][2][0][0]=50;
//	printf("t_start was filled with 50.....\n");
		arr_ent = 1;
		current_hinge_span[section_num] = first_hinge_span[section_num];
		while ((*current_hinge_span[section_num]).nexthinge != NULL) {
...
//				printf("recording %d-th walk\n", i);
				for(j=0; j<=2; j++){
//					printf("recording %d-th coordinate\n", j);
//					printf("section_num=%lu, arr_ent=%lu, j=%d, i=%d\n", section_num, arr_ent, j, i);
//	t_start[1][2][0][0]=53;
//	printf("t_start was filled with 53.....\n");
//					printf("t_end[%lu][%lu][%d][%d] is going to be filled with = %d\n", section_num, arr_ent, j, i, current_hinge_span[section_num]->end[j][i]);
...
//					printf("t_start[%lu][%lu][%d][%d] is going to be filled with = %d\n", section_num, arr_ent, j, i, current_hinge_span[section_num]->start[j][i]);
...
//					t_start[section_num][arr_ent][j][i] = 1;
				}
//				printf("filled t_start and t_end (i=%d)\n", i);
				for(j=0; j<=vM*vL; j++){
					t_walks[section_num][arr_ent][j][i] = current_hinge_span[section_num]->walks[j][i];
//					printf("t_walks[%lu][%lu][%d][%d] = %d\n", section_num, arr_ent, j, i, t_walks[section_num][arr_ent][j][i]);
				}
//				printf("filled t_walks (i=%d)\n", i);
			}
			arr_ent++;
		}
	}
...
/***** start of function conv_endhinges_to_array *************************************/
/* called only by main */
void conv_endhinges_to_array(void)
{
	unsigned long int section_num;
	...
	//fill Lend_num_walks and Rend_num_walks
	for (section_num = 1; section_num <= max_sections; section_num++) {
		L_arr_ent = 1;
		R_arr_ent = 1;
		currentendhinge[section_num] = firstendhinge[section_num];
...
/* it returns 1 if the two-span being considered connects to phi on the left and returns 0 otherwise */
/* it also uses the global variable ordertemplate */
#include "../topology/LFlag_endhinge_norder.c"
