/***************************************************************************/
unsigned long int
num_section_endhinge_norder3(void)
{
	...
	unsigned short int slot_num[11];	/*of the positions available to the edge determined by*/
...
for (i=1;i <= 10;i++){slot_num[i]=0;} /* This is necessary to initialize the array to contain zero elements */

	for (i = 0; i <= M; i++) {
		for (j = 0; j <= L; j++) {
			edge_num=rendtemplate3[i][j];/*assign number of edge determined by i and j to edge_num */
...
if (slot_num[9] !=0){
	return(559953.0*slot_num[1]-631288.0+62217.0*slot_num[2]+7777.0*slot_num[3]+1111.0*slot_num[4]+185.0*slot_num[5]+37.0*slot_num[6]+9.0*slot_num[7]+3.0*slot_num[8]+slot_num[9]);
}
if (slot_num[7] !=0){
	return(559953.0*slot_num[1]-631288.0+62217.0*slot_num[2]+7777.0*slot_num[3]+1111.0*slot_num[4]+185.0*slot_num[5]+37.0*slot_num[6]+9.0*slot_num[7]+3.0*slot_num[8]);
}
if (slot_num[5] !=0){
	return(559953.0*slot_num[1]-631277.0+62217.0*slot_num[2]+7777.0*slot_num[3]+1111.0*slot_num[4]+185.0*slot_num[5]+37.0*slot_num[6]);
}
if (slot_num[3] !=0){
	return(559953.0*slot_num[1]-631056.0+62217.0*slot_num[2]+7777.0*slot_num[3]+1111.0*slot_num[4]);
}
	return(559953.0*slot_num[1]-622169.0+62217.0*slot_num[2]);
...
/***************************************************************************/
unsigned long int
num_section_endhinge_norder4(void)
{
	unsigned long int i,j; 			/*used as dummy variables in for loops */
	unsigned short int edge_num;		/*the number of the edge being considered in for loop*/
						/*this is zero if there is no edge there */
...
for (i=1;i <= 10;i++){slot_num[i]=0;} /* This is necessary to initialize the array to contain zero elements */
	...
	for (i = 0; i <= M; i++) {
		for (j = 0; j <= L; j++) {
			edge_num=rendtemplate4[i][j];/*assign number of edge determined by i and j to edge_num */
							   /*this will be zero if there is no edge */
