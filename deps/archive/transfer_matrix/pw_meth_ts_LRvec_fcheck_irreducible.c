...
// irreducible version, meaning when it connects two 2-spans, must check if it forms a hook
...
/**HERE**/
			...
			/*loop through sections which connect on the right*/
				...
				//only do this if irreducible
				//only do this if row_tspan_num can connect to col_tspan_num
				canconnect=0;
				for(i=1; i<=num_outtspans[row_tspan_num]; i++){
					if(ConnectInfo[row_tspan_num][i]==col_tspan_num){
						canconnect=1;
						break;
					}
				}
