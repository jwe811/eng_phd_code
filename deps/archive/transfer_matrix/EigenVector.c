...
#define x_L 0.596565			/* left end point to use in bracketing method to find x_0        */
#define x_R 0.596567			/* right end point to use in bracketing method to find x_0       */
...
char filename[36];
FILE* fp;
sprintf(filename,"RvectorL%dM%dHam.txt",L,M);
fp = fopen(filename,"w");

if(fp == NULL)
  {
  printf("File is pointing to null\n");
  exit(1);
  }
