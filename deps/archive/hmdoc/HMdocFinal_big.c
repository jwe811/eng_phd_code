//Aug 15, 2016
//This version will put a convention on the orientation of the edges in the 2-span. This will result in less section numbers, but more connections between
//the section numbers.
//Section orderings will be mapped to having (0,0) as the first edge, if it's there. If not, (1,0) and so on.
//Hopefully will allow for smaller "max_sections" which means hopefully 5x1 and 3x2 is possible (12 slots)

//use reorderedtemplate[2][vM][vL] to follow convention.
...
#define x_L 0.4335			/* left end point to use in bracketing method to find x_0        */
...
*/
	...
	printf("maxobs=%d\n", maxobs);
	printf("totalsecs=%d\n", totalsecs);
	printf("total2spans=%d\n", total2spans);


/*	for(i=1; i<=max_sections; i++){
		if(num_outsections[i]>0){
			printf("Section %d connects to sections: ", i);
			for(j=1; j<=num_outsections[i]; j++){
...
//	printf("leavehinge called\n");
	...
	/*
				...
				while(recordedsections[ii]!=0){
					if(recordedsections[ii] == num_section_norder(0)){
						match=1;
						break;
					}
					ii++;
				...
				if(match==0){	//havent recorded this section yet
					recordedsections[ii]=num_section_norder(0);	//record the section
	...
	/*		else{
				if(LFlag(pointordNum) != 1){
					...
					while(failedsections[ii]!=0){
						if(failedsections[ii]==num_section_norder(0)){	//already recorded this failed section
							failedmatch=1;
							break;
						}
						ii++;
					...
					if(failedmatch==0){
						failedsections[ii]=num_section_norder(0);
						if((*pointordNum)[0]==3){	//2 edges in left section
							twoedgethrown++;
						}
						else if((*pointordNum)[0]==5){	//4 edges in left section
	...
	*/		num_2_spans++; /*Keep track of all 2 spans, valid and non-valid, just for interest sake*/
	...
	//			printf("trying to re-enter at (%d, %d)\n", jj, ii);
...
byteloc = (num_section_norder(0)/8);
bitloc  = (num_section_norder(0)%8);
switch(bitloc) {
...
/* which has been verified as being valid into a linked list */
{
	...
	(*current_hinge_span[inNum]).outorder = num_section_norder(1);	/* record the ordering on the out side */
...
#if vM*vL<7
#include "../sections/Num_section_6V_nonordered.c"  /*unsigned long int num_section(int side)*/
...
//					printf("odd edge found in (%d,%d)\n", j, i);
					reordertemplate[s][i][j] = nextentry;
//					printf("recorded edge %d in reorder\n", nextentry);
					nextentry++;
					for(ii=0; ii<=M; ii++){
						for(jj=0; jj<=L; jj++){
							if(ordertemplate[s][ii][jj]==ordertemplate[s][i][j]+1){
//								printf("found next edge. located at (%d, %d)\n", jj, ii);
								reordertemplate[s][ii][jj] = nextentry;
								nextentry++;
								break;
							}
						}
...
//					printf("even edge found in (%d,%d)\n", j, i);
					reordertemplate[s][i][j] = nextentry+1;
//					printf("recorded edge %d in reorder\n", nextentry);
					for(ii=0; ii<=M; ii++){
						for(jj=0; jj<=L; jj++){
							if(ordertemplate[s][ii][jj]==ordertemplate[s][i][j]-1){
