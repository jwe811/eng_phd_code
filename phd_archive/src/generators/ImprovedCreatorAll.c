...
//			printf("finished at i=%d, j=%d\n", i, j);
	...
	EndOrdNum[0]=1;	//first edge in right section will get the label 1
...
/* called by: rowedges,coledges,enterhinge */
{
	...
	//if side==0, then we are exiting to the left, so we are in direction negx (2) (also add -333 for exiting left)
	//else, we are exiting to side==1 (right) (posx, 1) (also add -222 for exiting right)
	if(side==0){
		curwalk[index]=2;
		curwalk[index+1]=-333;
	}
