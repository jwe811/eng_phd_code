...
//THIS VERSION (twostrings) will only print polys with 2 edges in first/last sections, and >2 edges in middle sections
...
//				printf("i=%d, minedges=%d\n", i, minedges);
				if(i>=minedges){
					...
					if(filetotal>maxpolys){	//then we need to close up the current file, and create a new one to write to.
						...
						fclose(fp);
						filenum++;
						sprintf(filename, "CreatorTwoStrings_PKshrunkPolysL%dM%dspan%dnum%d.txt", passedL, passedM, totalspan, filenum);
