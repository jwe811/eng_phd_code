...
//				printf("GOT TO HERE. POLYGON IS CREATED\n");
				...
				//CHECK HOW MANY EDGES ARE IN THE POLY. IF >=minedges, THEN WE PRINT IT.
				i=0;
				while(printablewalk[i] >0){
					i++;
				}
//				printf("this poly has %d edges\n", i);
				...
				if(i>=minedges){
					...
					if(filetotal>maxpolys){	//then we need to close up the current file, and create a new one to write to.
						...
						fclose(fp);
						filenum++;
						sprintf(filename, "CreatorAll_InterestPKPolysL%dM%dspan%dnum%d.txt", passedL, passedM, totalspan, filenum);
