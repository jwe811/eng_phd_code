...
// only generates polygons with "patterns of interest". This means their start and end sections only have two edges. Polygon must go the full span.
//ALSO-polys must have a min number of edges. 	
...
//WORKS FOR 2X2 (SPAN3) Oct 7, 2015


//changes in v3:	Only interested in patterns, so only generate each pattern with one end-hinge, instead of all of them.
...
#define	totalspan 9		/* span of the polygon that will be generated (must be >=3)	*/
...
/************************************************************************************************/
	...
	sprintf(filename, "CreatorPrime_InterestPKshrunkPolysL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
	...
	sprintf(trackingname, "tracking_CrPrIntPKshrL%dM%dspan%d.txt", L, M, totalspan);
...
#include "recursivegenerate_interestPKshrunk.c"
