/*	This program will create all COMPRESSED polygons in a LxM tube, with chosen span. Polygon must go the full span.	*/
/*	Special 2x2 version because of periodic nature.	*/
...
//NOTE: THESE ARE FULL-HINGE VERSION NUMBERS
#if (M==1 && L==1)
#define maxendhinges 7
#elif (M==1 && L==2)
#define maxendhinges 43
#elif (M==1 && L==3)
#define maxendhinges 277
#elif (M==2 && L==2)
#define maxendhinges 969
#endif
...
unsigned long int found_sections[max_sections];	//list of section numbers where the endhinge connection info has already been found
unsigned long int num_con_left[max_sections];	//entries corresponding to found_sections, the num of sections that connect to the left
unsigned long int num_con_right[max_sections];	//entries corresponding to found_sections, the num of sections that connect to the right
unsigned long int* con_left[max_sections];		//first index will be 0, 1, 2, ..., 0, 0, ...     second index will be 1, 2, ..., num_con_left[firstindex], will contain endhinge number that connects on the left
unsigned long int* con_right[max_sections];		//first index will be 0, 1, 2, ..., 0, 0, ...     second index will be 1, 2, ..., num_con_left[firstindex], will contain endhinge number that connects on the left
...
/************************************************************************************************/
	...
	sprintf(filename, "CreatorPrimePolysL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
	...
	for(firstsection=1; firstsection<=max_sections; firstsection++){
	  //printf("firstsection=%d, ", firstsection);
...
*/
	...
	*/
	...
	if(filenum==1){
	  ...
	  printf("GRANDTOTAL=%d!\n", filetotal + ((filenum-1)*(maxpolys+1)));
	}

	printf("grand=%d\n", grand);

	printf("\nDone generating all polygons in %f seconds.\n\n",(double)clock()/(double)CLOCKS_PER_SEC);
	fclose(fp);
...
//						printf("found a 2span, but not all vertices are filled\n");
						fullhinge=0;
						break;
					}
				}
			...
			if(fullhinge==1){
//				printf("found a 2span with all vertices filled\n");
...
//	printf("inNum=%d, outNum=%d. \n", inNum, (*current_hinge_span[inNum]).outorder);
...
//		printf("%d, ", curwalk[i]);
	...
	//check if this section has already been done
	int index=0;
	while(found_sections[index]!=0){
		if(found_sections[index]==inNum){	//already done section inNum (stored at index)
//			printf("Section %d has already been done\n", inNum);
			alreadydone=1;
			break;
		}
		else{
	...
	//	printf("there are %d endhinges that connect to that left side of ordertemplate\n", num_con_endhinges_left);
		...
		unsigned long int index2=0;		//index of con_edges_left
		...
		if(num_con_endhinges_left==0){
			...
			for(i=0; i<=num_con_endhinges_left-1; i++){	//loop through all temp[i=0...num_con_endhinges_left-1]
				//find the corresponding endhinge that contains the same edges as temp[i], then add the endhinge number to con_edges_left[index2]
	...
	//	printf("\n");
		...
		//record endhinge info for later use
		i=0;
		while(found_sections[i]!=0){
			i++;
		}
		found_sections[i]=inNum;
		num_con_left[i]=num_con_endhinges_left;
		if(num_con_left[i]!=0){
			con_left[i] = unsgnlong_vecalloc(0, num_con_endhinges_left-1);
			for(j=0; j<=num_con_endhinges_left-1; j++){
				con_left[i][j] = con_edges_left[j];
			}
		}

		num_con_right[i]=num_con_endhinges_right;
		if(num_con_right[i]!=0){
			con_right[i] = unsgnlong_vecalloc(0, num_con_endhinges_right-1);
			for(j=0; j<=num_con_endhinges_right-1; j++){
				con_right[i][j] = con_edges_right[j];
			}
		}

	}
	else{	//already have endhinge info for this section
//		printf("using already obtained info\n");
//		printf("section %d connects to %d endhinges on the left\n", inNum, num_con_left[index]);
		current_hinge_span[inNum]->num_con_endhinges_left = num_con_left[index];
		if(num_con_left[index]>0){
			current_hinge_span[inNum]->con_endhinges_left = unsgnlong_vecalloc(0, num_con_left[index]-1);	//list of endhinge "numbers" that connect to the left of section inNum
			for(i=0; i<=num_con_left[index]-1; i++){
				current_hinge_span[inNum]->con_endhinges_left[i] = con_left[index][i];
			}
		}

//		printf("section %d connects to %d endhinges on the right\n", inNum, num_con_right[index]);
		current_hinge_span[inNum]->num_con_endhinges_right = num_con_right[index];
		if(num_con_right[index]>0){
			current_hinge_span[inNum]->con_endhinges_right = unsgnlong_vecalloc(0, num_con_right[index]-1);	//list of endhinge "numbers" that connect to the left of section inNum
			for(i=0; i<=num_con_right[index]-1; i++){
				current_hinge_span[inNum]->con_endhinges_right[i] = con_right[index][i];
			}
		}
	}
