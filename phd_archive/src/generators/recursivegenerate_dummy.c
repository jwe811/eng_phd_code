...
//	printf("adding a 2-span to printablewalk (sections %d and %d make up to 2-span)\n", firstsec, tspantoadd->outorder);
				...
				while(valid_secs[i]!=0){
					if(tspantoadd->outorder==valid_secs[i]){
						match=1;
						break;
					}
					i++;
				...
				if(match==0){
//					printf("adding section %d to valid_secs[%d]\n", tspantoadd->outorder, i);
					valid_secs[i]=tspantoadd->outorder;
				}
...
//DONT NEED TO PRINT SINCE THIS IS DUMMY
/*
				fprintf(fp, "%d %d 0\n", x, y);	//starting point
				i=0;
				while(printablewalk[i] != 0){
...
*/
				...
				if(filetotal>maxpolys){	//then we need to close up the current file, and create a new one to write to.
					...
					fclose(fp);
					filenum++;
	//ALWAYS dummy.txt BECAUSE THIS IS A COUNTER
