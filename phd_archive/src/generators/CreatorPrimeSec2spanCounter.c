...
int valid_secs[max_sections+1];	//holds all valid sections
...
//POINT fp TO DUMMY TEXT FILE BECAUSE THIS IS THE COUNTER
	sprintf(filename, "dummy.txt");
	...
	for(firstsection=1; firstsection<=max_sections; firstsection++){
		...
		while((*current_hinge_span[firstsection]).nexthinge != NULL){
			...
			if((*current_hinge_span[firstsection]).num_con_endhinges_left > 0){	//this tests to make sure we have a VALID starting 2-span (has a connecting endhinge on the left)
				...
				i=0;	//add valid section to array if not already added
				match=0;
				while(valid_secs[i]!=0){
					if(firstsection==valid_secs[i]){
						match=1;
						break;
					}
					i++;
				...
				if(match==0){
//					printf("adding section %d to valid_secs[%d]\n", firstsection, i);
					valid_secs[i]=firstsection;
				}
...
//KEEPING ALL DUPLICATES BECAUSE THIS IS THE COUNTER
...
*/
					...
					//next, add appropriate steps to printablewalk[] for all possible starting endhinges
...
//					printf("adding section %d to valid_secs[%d]\n", nextsection, i);
	...
	//count number of valid starting sections (count number of entries in valid_secs)
	int num_valid_secs=0;
	i=0;
	while(valid_secs[i]!=0){
		num_valid_secs++;
		i++;
	}
	printf("number of valid starting sections=%d\n", num_valid_secs);
	...
	//count number of valid 2spans (for each valid section, count number of 2-spans starting with that section)
	i=0;
	int num_valid_tspans=0;
	while(valid_secs[i]!=0){
		current_hinge_span[valid_secs[i]] = first_hinge_span[valid_secs[i]];
		while ((*current_hinge_span[valid_secs[i]]).nexthinge != NULL) {
			current_hinge_span[valid_secs[i]] = (*current_hinge_span[valid_secs[i]]).nexthinge;
			num_valid_tspans++; /*count that number of two-spans with valid_secs[i] as first section*/

		}
		i++;
	}
	printf("number of valid 2spans=%d\n", num_valid_tspans);
...
#include "recursivegenerate_dummy.c"
