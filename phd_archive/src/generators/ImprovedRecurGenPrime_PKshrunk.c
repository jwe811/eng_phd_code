...
//			printf("%dth section=%d\n", curspan+1, nexttspan->outorder);
		...
		//close off polygon with all possible endhinges.
...
//				printf("this poly has %d edges\n", i);
				...
				if(i>=minedges){
					...
					grand++;
					filetotal++;;
					printf("recorded a poly (grand=%d)\n", grand);
					...
					if(filetotal>=maxpolys){	//then we need to close up the current file, and create a new one to write to.
						...
						fclose(fp);
						filenum++;
						sprintf(filename, "ImpCreatorPrime_PKshrunkPolysL%dM%dspan%dnum%d.txt", passedL, passedM, totalspan, filenum);
