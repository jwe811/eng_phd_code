//This program will loop through different values of f and calculate probabilities of patterns in file.
...
#define force -10			/* fixed force in model. */	//changeable - initial force; will be incremented
#define maxforce 10			//changeable --the max force in the interation
#define increment 0.5		//changeable --the increment of force added after each interation



//#define	connectivity_inv 0.044944	/* When the connectivity is known its inverse is defined here.   */
...
#define x_L 0.2			/* left end point to use in bracketing method to find x_0        */ //changeable //initial leftbound-should correspond to initial force
#define x_R 0.8			/* right end point to use in bracketing method to find x_0       */ //changeable //initial rightbound-should correspond to initial force
...
double leftbound = x_L;
double rightbound = x_R;
...
double			probkspan(double, double, double, FILE*);
double			probkspan2(double, double, double, FILE*);
double			probkspan3(double, double, double, FILE*);
...
//Full version runs check here, for irreduciblity and "actual" valid 2span/sections counts
	...
	double p1, p2, p3;
	//changeable
	FILE* fp = fopen("GlobalLocal2x1TwoStringProbs_f.csv", "w");	//this file will be where we record probabilities
	if(fp != NULL){
		printf("Opened GlobalLocal2x1TwoStringProbs_f.csv\n");
	}
	else{
		printf("file pointer is pointing to NULL, couldn't open GlobalLocal2x1TwoStringProbs_f.csv. Exiting program.\n");
		exit(1);
	}
	fprintf(fp, "g f LocalProb GlobalProb Global2Prob\n");	//header row

	//changeable
	for(fval=force; fval<=maxforce; fval = fval + increment){
		printf("starting loop with f=%f\n", fval);
		//update leftbound and rightbound here
		//for 2x1 tube, we have the relationship of approx x_0 = 0.4028264 * e^-0.2220786 (f)
		leftbound = 0.4028264 * exp(-0.2220786 * fval);
		leftbound = leftbound - leftbound/3;
		rightbound = 0.4028264 * exp(-0.2220786 * fval);
		rightbound = rightbound + rightbound/3;
...
//		printf("Starting false position method to determine connective constant.\n");
		#if defined(force)
			printf("=> %f=%e is fugacity which yields 1 as maximum eigenvalue (after multiplying matrix by e^%f)\n",
						connectivity_inv=rtflsp(&max_eval_LRvec,leftbound,rightbound, 0.0000001), connectivity_inv, fval);
		#else
			printf("=> %f is fugacity which yields 1 as maximum eigenvalue\n",
						connectivity_inv=rtflsp(&max_eval_LRvec,leftbound,rightbound, 0.0000001));
		#endif
//			printf("=> %f is the connective constant.\n",1/connectivity_inv);
	#endif
...
//		printf("L_Evector[phi]*R_Evector[phi] = %f = %e\n\n",Ephi_Ephi,Ephi_Ephi);
	...
	//changeable
	fprintf(fp, "%.1f %f ", fval, kappa );

		p1 = probkspan(Beta, LmultR, kappa, fp);
		p2 = probkspan2(Beta, LmultR, kappa, fp);
		p3 = probkspan3(Beta, LmultR, kappa, fp);
	...
	//	double expoccurtwostrings = avgtwostrings(Beta, LmultR, kappa, loopforce);
...
#if defined(SS)
	#if defined(force)
		#include "../../transfer_matrix/pw_meth_ss_LRvec_fcheck_loopedf.c" /* for applying the power method to the section-section matrix */ //changeable
		#include "get_Beta_ssCheck.c"	/* for calculating Beta */
	#else
		#include "../../transfer_matrix/pw_meth_ss_LRvec.c" /* for applying the power method to the section-section matrix */
		#include "get_Beta_ss.c"	/* for calculating Beta */
	#endif
...
#if defined(TS)
#include "sumtoone_ts.c"
//#include "../../sections/enterpatstspan_ts.c"
#include "../../sections/enterpats_ts.c"
#include "transprobcheck_ts.c"
#include "probkspan_ts.c"
#else
#include "sumtoone_ss.c"
#include "prob_check_ss.c"
//#include "../../sections/enterpatstspan_ss.c"
#include "../../sections/enterpats_ss.c"
#include "transprobcheck_ss.c"
#include "probkspan_ss_looped.c"
#include "probkspan_ss_looped2.c"
#include "probkspan_ss_looped3.c"
...
#include "graphedgesvsnumconnect.c"
#include "graphedgesvsconnectto.c"
#include "graphedgesvstotalconnect.c"
#endif
...
#include "rtflsp.c"	/*double rtflsp(double(*func)(double),double x1,double x2,double xacc)*/ //changeable
