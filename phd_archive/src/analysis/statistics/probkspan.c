...
//	Edges[2] = #edges in k-span (not including last section)
	...
	while(filenum<=totalfilenum){	//while there are files to read in
		...
		sprintf(filename, "CreatorPrimePolysL%dM%dspan%dnum%d_3_1s.txt", L, M, totalspan, filenum);
