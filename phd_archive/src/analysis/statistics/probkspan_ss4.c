//TUBE IN X-DIRECTION probkspan1-3 has tube in z-direction

void probkspan4(double Beta, double LmultR, double kappa)
{
	printf("probkspan4() (section version) is running!\n");

	//CHANGEABLE THINGS HERE: (ctrl+F changeable to find other changeable things)
	//total number of files that need to be read in.
	int totalfilenum = 1;
	int totalspan = 5;	//span of polygons being looked at
...
//	Edges[2] = #edges in k-span (not including last section)
	...
	while(filenum<=totalfilenum){	//while there are files to read in
		...
		sprintf(filename, "TwoStringsFullButNotHamL%dM%dspan%dnum%d.txt", L, M, totalspan, filenum);
...
//			printf("starting point is %d, %d, %d\n", x, y, z);
			...
			while(1){	//while still in the polygon
				...
				if(direc==5){
					z=z+1;
					if(x!=0 && x!=totalspan){	//if not in an endhinge
						Edges[2]++;
					}
				}
				else if(direc==6){
					...
					x=x+1;
					if(x!=totalspan){	//if not in last section
						Edges[2]++;
					}
				}
				else if(direc==2){
					x=x-1;
					if(x!=(totalspan-1)){	//if not in last section
						Edges[2]++;
					}
				}
			}
