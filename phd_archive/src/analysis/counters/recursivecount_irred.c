...
//				printf("GOT TO HERE. POLYGON IS CREATED\n");
				//must do final check to see if it's irreducible
				i=0;
				while(printablewalk[i]>0){	//copywalk doesnt have the -111 at the end
					copywalk[i]=printablewalk[i];
					i++;
				}
				polylength=i;
				for(j=i; j<=pwalksize-1; j++){
					copywalk[j]=0;
				}
				
				if(hookedwalk(copywalk)==0){
					if(bfacf_irred_check(x, y, 0, copywalk, polylength)==0){	//couldnt find a way to shrink
						if(polylength>=lower && polylength<=upper){
							printf("Found (printing) an irred poly with length between %d and %d (length=%d)\n", lower, upper, polylength);

							fp = fopen(filename, "a");	//open "filename" to record poly
							...
							if(filetotal>maxpolys){	//then we need to close up the current file, and create a new one to write to.
								...
								fclose(fp);
								filenum++;
								sprintf(filename, "MSNIrredPolysBetween_%d_%d_L%dM%dspan%dnum%d.txt", lower, upper, passedL, passedM, totalspan, filenum);
