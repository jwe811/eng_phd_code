/*	This program will create IRREDUCIBLE polygons in a LxM tube that fall within a range of length sizes (lower upper), with chosen span. Polygon must go the full span.	*/
...
#define	totalspan 14		/* span of the polygon that will be generated (must be >=3)	*/

#define 	maxpolys 100000	/* maximum number of polygons in each .txt file	*/
...
/************************************************************************************************/
	...
	sprintf(filename, "MSNIrredPolysBetween_%d_%d_L%dM%dspan%dnum%d.txt", lower, upper, L, M, totalspan, filenum);
	...
	sprintf(trackingname, "tracking_MSNcounterBetween_%d_%d_L%dM%dspan%d.txt", lower, upper, L, M, totalspan);
	fptwo = fopen(trackingname, "w");
	if(fptwo != NULL){
		fprintf(fptwo, "Starting to create polys\n");
		printf("tracking_MSNcounterBetween.....txt is created\n");
	}
	else{
...
*/
	...
	*/
	...
	//								printf("counter=%d\n", counter);
						...
						//check for hooks on the left here?
//						printf("Checking for hooks on the left (in printablewalk)\n");
						if(hookedhinge(printablewalk)==0){	//if no hooks on the left side, continue
//							printf("no hooks on left\n");
...
int hookedwalk(int hingewalk[]){	//returns 1 if the walk has a hook in it. 0 otherwise

	int i;
	int length;
//	printf("hookedwalk() is running\n");
...
*/
	...
	//must also check start/end edges form a hook
	//if we made it this far, we haven't found a hook. i+1 has the last non-zero entry
	length=i+1;
//	printf("length=%d\n", length);

	int one, two, three; //will be the indices are of the three edges.
...
////////////////////////////////////////////
	...
	if(hingewalk[one]==5 && hingewalk[two]==3 && hingewalk[three]==6){	//+z+y-z
		return 1;
	}
	else if(hingewalk[one]==5 && hingewalk[two]==4 && hingewalk[three]==6){	//+z-y-z
		return 1;
	}
	else if(hingewalk[one]==5 && hingewalk[two]==1 && hingewalk[three]==6){	//+z+x-z
		return 1;
	}
	else if(hingewalk[one]==5 && hingewalk[two]==2 && hingewalk[three]==6){	//+z-x-z
		return 1;
	}
/////
	else if(hingewalk[one]==6 && hingewalk[two]==3 && hingewalk[three]==5){	//-z+y+z
		return 1;
	}
	else if(hingewalk[one]==6 && hingewalk[two]==4 && hingewalk[three]==5){	//-z-y+z
		return 1;
	}
	else if(hingewalk[one]==6 && hingewalk[two]==1 && hingewalk[three]==5){	//-z+x+z
		return 1;
	}
	else if(hingewalk[one]==6 && hingewalk[two]==2 && hingewalk[three]==5){	//-z-x+z
		return 1;
	}
/////
	else if(hingewalk[one]==1 && hingewalk[two]==3 && hingewalk[three]==2){	//+x+y-x
		return 1;
	}
	else if(hingewalk[one]==1 && hingewalk[two]==4 && hingewalk[three]==2){	//+x-y-x
		return 1;
	}
	else if(hingewalk[one]==1 && hingewalk[two]==5 && hingewalk[three]==2){	//+x+z-x
		return 1;
	}
	else if(hingewalk[one]==1 && hingewalk[two]==6 && hingewalk[three]==2){	//+x-z-x
		return 1;
	}
/////
	else if(hingewalk[one]==2 && hingewalk[two]==3 && hingewalk[three]==1){	//-x+y+x
		return 1;
	}
	else if(hingewalk[one]==2 && hingewalk[two]==4 && hingewalk[three]==1){	//-x-y+x
		return 1;
	}
	else if(hingewalk[one]==2 && hingewalk[two]==5 && hingewalk[three]==1){	//-x+z+x
		return 1;
	}
	else if(hingewalk[one]==2 && hingewalk[two]==6 && hingewalk[three]==1){	//-x-z+x
		return 1;
	}
/////
	else if(hingewalk[one]==3 && hingewalk[two]==1 && hingewalk[three]==4){	//+y+x-y
		return 1;
	}
	else if(hingewalk[one]==3 && hingewalk[two]==2 && hingewalk[three]==4){	//+y-x-y
		return 1;
	}
	else if(hingewalk[one]==3 && hingewalk[two]==5 && hingewalk[three]==4){	//+y+z-y
		return 1;
	}
	else if(hingewalk[one]==3 && hingewalk[two]==6 && hingewalk[three]==4){	//+y-z-y
		return 1;
	}
/////
	else if(hingewalk[one]==4 && hingewalk[two]==1 && hingewalk[three]==3){	//-y+x+y
		return 1;
	}
	else if(hingewalk[one]==4 && hingewalk[two]==2 && hingewalk[three]==3){	//-y-x+y
		return 1;
	}
	else if(hingewalk[one]==4 && hingewalk[two]==5 && hingewalk[three]==3){	//-y+z+y
		return 1;
	}
	else if(hingewalk[one]==4 && hingewalk[two]==6 && hingewalk[three]==3){	//-y-z+y
		return 1;
	}
...
#include "recursivecount_irred.c"
#include "../../generators/bfacf_irred_check.c"
