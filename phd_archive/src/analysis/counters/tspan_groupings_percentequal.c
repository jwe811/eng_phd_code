void groupings(double Beta,double LmultR,double kappa) {
	...
	*/
	...
	double tolerance=0.00000000001;	//take percentage difference when comparing expected number of occurrences. anything that is less that this
								//tolerance will be considered equal

	printf("STARTING groupings() (percent equal)\n");
	...
	int firstindextotal=2000;
	int secondindextotal=50;
...
//			printf("j=%d\n", j);
			if((fabs(exp_occur[i]-exp_occur[j])/((exp_occur[i]+exp_occur[j])/2))<tolerance){	//tspan i matches tspan j within a certain percentage tolerance
...
//		printf("\n");
		...
		if(m==40){
			printf("group %d: ", curgroup);
			for(i=0; i<=secondindextotal-1; i++){
				if(groups[curgroup][i]!=0){
					printf("%d, ", groups[curgroup][i]);
				}
				else{
					break;
				}
