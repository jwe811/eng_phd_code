/*
This program will output two text files--one containing the start-hinge section transfer matrix, with the other
containing the start-hinge 2span transfer matrix (for the specified tube size).
The text files will be called StartHingeSectionTransferMatrixL%dM%d.txt and StartHingeTspanTransferMatrixL%dM%d.txt
*/
...
int num_left_endhinges[vec_length]; //given a section numbers (index), gives the number of endhinges which connect on the left
unsigned long int *endhinge_labels[vec_length];	//given a section number, nth entry is the nth left endhinge number.
...
/************************************************************************************************/
	...
	//PRINT START-HINGE SECTION TRANSFER MATRIX

	sprintf(filename, "StartHingeSectionTransferMatrixL%dM%d.txt", L, M);
	...
	int connects;
	for(i=0; i<=total_end_hinges-1; i++){ //row
		for(j=1; j<=max_sections; j++){
			if(num_outsections[j]>0){ //column
				connects=0;
				for(k=1; k<=num_left_endhinges[j]; k++){
					if(i==endhinge_labels[j][k]){
						connects=1;
					}
				}
				if(connects==0){
					...
					fprintf(fp, "x^%d ", numedgesinendhinge(i));
				}
			}
		}
		fprintf(fp,"\n");
	...
	fclose(fp);
	//PRINT START-HINGE 2SPAN TRANSFER MATRIX

	sprintf(filename, "StartHingeTspanTransferMatrixL%dM%d.txt", L, M);
	fp = fopen(filename, "w");	//create or overwrite the file "filename"
...
conv_to_array(void) /* tspans_outsection[section_num][1..num_outsections[section_num]] */
{
	...
	for (section_num = 1; section_num <= max_sections; section_num++) {
		...
		num_left_endhinges[section_num] = current_hinge_span[section_num]->num_con_endhinges_left;
		...
		endhinge_labels[section_num] = unsgnlong_vecalloc(1, num_left_endhinges[section_num]);
		for(i=1; i<=num_left_endhinges[section_num]; i++){
			endhinge_labels[section_num][i] = current_hinge_span[section_num]->con_endhinges_left[i-1];
		}
