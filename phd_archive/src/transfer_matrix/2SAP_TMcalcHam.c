//This generates and calculates dom. eigenvalue/eigenvectors for for 2SAPs.
...
#define x_L 0.5	/* left end point to use in bracketing method to find x_0        */
...
//		printf("Taking out section %d, k=%d of %d\n", nonconnectingsections[k], k, numnoconnect-1);
	...
	L_Evector[0] = vecalloc(0,max_keynum); /*global*/
	L_Evector[1] = vecalloc(0,max_keynum); /*global*/ /*temp*/
	R_Evector[0] = vecalloc(0,max_keynum); /*global*/
	R_Evector[1] = vecalloc(0,max_keynum); /*global*/ /*temp*/
...
//	printf("R_Evector[0][1]=%f\n", R_Evector[0][1]);
	...
	printf("Final number of saved 2-spans (before removal)=%lu\n", total2spans);
...
#if L==2 && M==2
	#include "../experiments/Trying_2SAP.c" /* for applying the power method to the section-section matrix */
#else
	#include "pw_meth_ss_LRvec_fcheck_2SAP.c" /* for applying the power method to the section-section matrix */
#endif

#include "../analysis/statistics/get_Beta_ssCheck_2SAP.c"	/* for calculating Beta */
