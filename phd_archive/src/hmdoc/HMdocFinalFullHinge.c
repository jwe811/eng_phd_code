/*
WORKS FOR 2x2 (uses Trying.c for pw method)--Aug 3, 2016. Don't need HMdocFinalFullHinge2x2.c anymore.
...
//#define	thex 0.163125	/* When the connectivity is known its inverse (x_0) is defined here.   */

#define x_L 0.597402			/* left end point to use in bracketing method to find x_0        */
#define x_R 0.597404			/* right end point to use in bracketing method to find x_0       */
...
void				probkspan(double, double, double); //tube in z-direction
void				probkspan2(double, double, double); //tube in z-direction
void				probkspan3(double, double, double); //tube in z-direction
void				probkspan4(double, double, double); //tube in x-direction
void				probkspan5(double, double, double); //tube in x-direction
...
//	unsigned long int temptestvector[vec_length];	//used for the vector-matrix multiplication. had to make this global for some reason
	...
	//BE CAREFUL HERE BECAUSE OF PERIODICITY. NEED testpower TO BE BOTH EVEN AND ODD.
	...
	printf("Performing check with test power (%d) \n", testpower);
...
//		printf("iter=%d\n", k);
	...
	if(M==2 && L==2){
		...
		printf("Now performing with odd test power (%d)\n", testpower);
...
//	printf("Section %d doesn't connect to sections: \n", testsection);
	...
	for(i=1; i<=max_sections; i++){	//loop through all valid sections
		if(num_outsections[i]>0) {
			if(M==2 && L==2){
				if(testvector[i] == 0 && testvector2[i] == 0){	// check if testvector[] is 0 (if it's zero, then this section doesnt connect to testsection
		//			printf("%d,", i);
					numnoconnect++;
...
//	printf("oddeven=%d\n", oddeven);
	printf("\n\n");
	printf("ACTUAL 'VALID' SECTIONS FOR THE L=%d, M=%d COMPRESSED CASE IS: %d-%d=%d=%d\n", L, M, valid_sections, numnoconnect, valid_sections-numnoconnect,numconnect);
	...
	printf("must remove %d invalid sections\n", numnoconnect);
...
//	probkspan4(Beta, LmultR, kappa);
//	probkspan5(Beta, LmultR, kappa);
...
/*
	ordertemplate[0][0][0] = 0;
	ordertemplate[0][0][1] = 0;
	ordertemplate[0][0][2] = 0;
	ordertemplate[0][1][0] = 2;
	ordertemplate[0][1][1] = 0;
	ordertemplate[0][1][2] = 1;
	int ces;
	ces = num_section(0);
...
#include "../analysis/statistics/probkspan_ss4.c"
#include "../analysis/statistics/probkspan_ss5.c"
