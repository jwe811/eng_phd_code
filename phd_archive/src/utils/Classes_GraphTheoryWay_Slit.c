...
#define	L 8				/* number of horizontal edges                              */
#define	M 0				/* number of vertical edges                                */

#define force 0				/* fixed force. */
#define tstsec 10		//class one contains this section.
...
#define x_L 0.6		/* left end point to use in bracketing method to find x_0        */
#define x_R 1		/* right end point to use in bracketing method to find x_0       */
...
unsigned long int outs[max_sections+1];		//number of out paths from this section (index) in the tree
unsigned long int ConnSecs[max_sections+1];
...
//WANT TO FIND CONNECTED CLASSES.
//WILL FIND WHICH SECTIONS ARE IN CLASS_ONE
...
*/
	...
	unsigned long int testsection=tstsec;	//A section in "class one"
...
//	unsigned long int outs[max_sections+1];		//number of out paths from this section (index) in the tree (if zero, then we know its a leaf)
	...
	//find a section which is not in class one
	int count=0;
	for(i=1; i<=max_sections; i++){
		if(ConnectingSections[i]==0 && num_outsections[i]>0){
...
/*
	printf("Sections which are 'leaves': ");
	for(i=1; i<=max_sections; i++){
	...
	//check if each section that's a leaf can connect to section tstsec.

	int can=0;	//indicates if the leaf can connect to tstsec
	for(k=1; k<=max_sections; k++){
		if(ConnectingSections[k]==1 && outs[k]==0){	//k is a section that's a leaf.
			for(i=1; i<=max_sections; i++){
				ConnSecs[i]=0;
			}
			can=0;
			...
			while(updated==1){
				...
				for(i=1; i<=max_sections; i++){
					if(ConnSecs[i]==1){
						for(j=1; j<=num_outsections[i]; j++){
							if(tspans_outsection[i][j]==tstsec){
								can=1;
								break;
							}
							else if(ConnSecs[tspans_outsection[i][j]]==0){
								updated=1;
								ConnSecs[tspans_outsection[i][j]]=1;
							}
						}
					}
				}
...
//				printf("leaf %d can connect to section %d\n", k, tstsec);
				...
				printf("leaf %d CANNOT CONNECT to section %d\n", k, tstsec);
				exit(1);
			}
		}
	}
...
//NOW RUN POWER METHOD FOR CLASS ONE

//Class one:
	//only keep section in matrix if its in ConnectingSections[] 
	...
	int num_class_one = secs_in_class_one;
	int num_remove = valid_sections - num_class_one;
	int keep=0;	//1 if its in ConnectingSections
	...
	for(k=1; k<=max_sections; k++){
		if(num_outsections[k]>0){	//if a valid section #
			if(ConnectingSections[k]==0){	//section k is NOT in class one, then remove it.
				//printf("Taking out section %d\n", k);
				//printsection(k);
				for(j=1; j<=num_outsections[k]; j++){
					tspans_edges[k][j] = 0;						//CHANGED EDGES
					tspans_outsection[k][j] = 0;					//CHANGED OUTSECTION
				}
				num_outsections[k]=0;                            //CHANGED NUM_OUT
