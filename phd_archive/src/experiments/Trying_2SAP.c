/***************************************************************************/
//TRY USING THE SQUARE OF THE TRANSFER MATRIX INSTEAD OF JUST THE TRANSFER MATRIX
...
for (i=1;i<=max_keynum;i++) { L_Evector[0][i]=1; R_Evector[0][i]=1;}
...
/**/	/* L_Evector[1] = L_Evector[0] * Matrix */
	...
	for (i = 1; i <= max_keynum; i++) { /*initialize resultant vectors to zero */
		L_Evector[1][i] = 0;
		R_Evector[1][i] = 0;
	}

	for (insection_num=1;insection_num<=max_keynum;insection_num++){
...
//			printf("insection=%d, outsection=%d, edges=%d\n", insection_num, nth_outsection, tspans_edges[insection_num][nth_outsection]);
	...
	//repeat (square of matrix)
	for (i = 1; i <= max_keynum; i++) { /*initialize resultant vectors to zero */
		L_Evector[0][i] = L_Evector[1][i];
		R_Evector[0][i] = R_Evector[1][i];
		L_Evector[1][i] = 0;
		R_Evector[1][i] = 0;
	}
...
//			printf("insection=%d, outsection=%d, edges=%d\n", insection_num, nth_outsection, tspans_edges[insection_num][nth_outsection]);
	...
	for (i=1;i<=max_keynum;i++){/*check for largest component in resultant vectors*/
	...
	for (i=1;i<=max_keynum;i++) { L_Evector[1][i] /= L_lambda_0; R_Evector[1][i] /= R_lambda_0; }
...
/**/	/* L_Evector[0] = L_Evector[1] * Matrix */
	...
	for (i=1;i<=max_keynum;i++){ L_Evector[0][i] = 0; R_Evector[0][i] = 0; }
	...
	for (i=1;i<=max_keynum;i++){
		L_Evector[1][i] = L_Evector[0][i];
		R_Evector[1][i] = R_Evector[0][i];
		L_Evector[0][i] = 0;
		R_Evector[0][i] = 0;
	}
	/*initialize resultant vectors to zero */
	...
	for (i=1;i<=max_keynum;i++) { L_Evector[0][i] /= L_lambda_1; R_Evector[0][i] /= R_lambda_1; }
	/* "normalize" the eigenvectors */
